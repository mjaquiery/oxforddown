...
gg$inf <- ggplot(tmp, aes(x = Advisor, y = adviceInfluence,
...
As Figure \@ref(fig:ac-acc-dots-r-performance)A shows, participants' accuracy scores for initial estimates were close to the target values (partly because participants whose accuracy scores diverged considerably were excluded).
...
For this analysis confidence was directonally coded so that final decision confidence was negative if the answer side changed between the initial estimate and final decision.
...
auroc <- acc %>%
  ...
  geom_point(alpha = .2) +
  geom_richtext(x = .75, y = .99, hjust = .5, vjust = 1, fill = NA,
                aes(label = d), 
                data = r) +
  scale_x_continuous(limits = c(.5, 1), expand = c(0,0), breaks = c(.5, .75, 1)) +
  scale_y_continuous(limits = c(.5, 1), expand = c(0,0), breaks = c(.5, .75, 1)) +
  # coord_fixed() +
  facet_wrap(~Response) +
  labs(x = 'Mean accuracy', y = 'Area under\nROC curve') +
  broken_axis_bottom +
  theme(panel.spacing.x = unit(2, 'lines'),
        plot.margin = unit(c(0, 0, 4, 0), 'lines'))

roc / auroc + plot_annotation(tag_levels = 'A')
...
tmp <- familiarization %>% 
  mutate(agree = if_else(adviceSide == initialAnswer, "Agree", "Disagree")) %>%
  group_by(pid, Advisor, agree) %>%
  summarise(influence = mean(adviceInfluenceCapped), .groups = "drop") %>%
  mutate(across(-influence, factor)) %>%
  select(pid, influence, agree, Advisor)
...
mm_inf <- marginalMeans(tmp, influence, pid, "Agree-Disagree")

message(glue("Interaction calculation: {mm_inf$.interactionExpression}"))

s <- summariseANOVA(aov_inf$ANOVA, mm_inf)
...
Participants were less confident on final decisions than on initial estimates (`r aov_conf$s[2]`), and the decrease over time was greatest for the trials where the initial estimate was incorrect (`r aov_conf$s[3]`).
...
roc <- roc.avg %>%
  ...
  ggplot(aes(x = CorrectFALSE, y = CorrectTRUE)) +
  geom_abline(slope = 1, intercept = 0, linetype = 'dashed') +
  geom_line(aes(group = pid), alpha = .2, colour = 'grey', data = roc) +
  geom_line() +
  geom_point() +
  geom_plot(aes(label = gg), x = 1, y = .025, vp.width = 2/3, data = auroc.gg) +
  scale_x_continuous(limits = c(0, 1), expand = c(0, .01)) +
...
# Patchwork plots together
roc / auroc.gg + plot_annotation(tag_levels = 'A')
...
Familiarization %>%
  mutate(
    Agree = factor(if_else(advisor0adviceSide == responseAnswerSide,
                           'Agree', 'Disagree')),
    `Initial estimate correct` = factor(if_else(responseAnswerSideCorrect, 
                                                'Correct', 'Incorrect')),
  ...
  geom_point(alpha = 0.1, aes(colour = `Initial estimate correct`)) +
