...
cors <- lmerTest::lmer(
  formula = pPickHigh ~ Feedback * Dimension + (1 | Difference),
  data = tmp
) %>% 
  summary() %>%
  .$coefficients %>%
  ...
  mutate(
    s = glue("{num2str(Estimate)}, t({num2str(df)})={num2str(`t value`)}, p={prop2str(`Pr(>|t|)`)}")
  )
...
\mccorrect{TODO: Difficult to LM this maybe due to observations? Is there a heirarchical logistic regression we can use in lmer/R? Current one treats Difference as a random factor and we want it to be fixed slope. Participants nested within Feedback, but Feedback is a factor we're interested in.}
