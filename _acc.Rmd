...
# Used later to compare with Dates
dot_rt_gg <- ggplot(tmp, aes(y = Participant, x = Time, fill = Event, colour = Event)) +
...
There was no evidence of participants' metacognitive sensitivity being correlated with their task performance (Figure \@ref(fig:ac-acc-dots-r-performance)E). 
...
###### Influence of advice {#ac-dots-r-influence}

The extent and manner of decision and confidence changes from before to after advice is an important indicator of the extent to which participants treat the advice as informative (Figure \@ref(fig:ac-acc-dots-r-performance)D).
...
As shown in Figure \@ref(fig:ac-acc-dots-r-performance)E, the level of influence was equivalent between advisors, with most participants being almost exactly equally influenced by both advisors.
\mccorrect{TODO: Do we want to perhaps look at the last half of the trials on these blocks for influence, so we can make the argument that they have had a chance to get to know the advisors a little? How long are the blocks?}
...
During these trials, participants either received feedback or did not receive feedback according to their experimental condition.
...
\mccorrect{NICK: Why 59 and not 60 percent as in the previous experiment? Is this number hard to fix with a reasonable number of trials? (vs. 6/10 agree when correct, 4/10 agree when incorrect). MATT: I've looked at this - it really was 59 but I cannot remember why; presumably to balance agreement rates a bit given average participant accuracy but I am not sure.}
...
\mccorrect{NICK: Maybe give the overall percent of trials excluded on this basis.}
...
\mccorrect{NICK: This table feels a bit pointless given that nobody was excluded. Cut?}
...
\mccorrect{NICK: RT analysis makes more sense now. Maybe cut the RT analysis above, and present the plots side-by-side here as parallel panels to facilitate comparison?}
...
Correspondingly, accuracy varied substantially across participants (Figure \@ref(fig:ac-acc-dates-r-accuracy)). 
Also apparent in (Figure \@ref(fig:ac-acc-dates-r-accuracy)), participants managed to improve their performance from their initial estimates to their final decisions with both advisors. 
...
\mccorrect{NICK: This does not seem to match what's in the figure -- only an improvement with the good advisor, it looks to me. Test of interaction?}
...
\mccorrect{NICK: Refer to the figure in the main text. (But maybe it's not essential to have a figure -- could just report relevant values in the text, depending on what you choose to include above for the dots task experiment.)Again, useful to comment on the across-participant variability in confidence. Also seems to be a bigger difference in confidence for final decisions. Interaction?}
...
Estimates of the participants' metacognitive abilities were highly variable, with many participants displaying below-chance metacognitive ability (Figure \@ref(fig:ac-acc-dates-r-roc)).
...
\mccorrect{NICK: I suggest just reporting the correlation values and stats in the main text, to reduce the number of figures.}
...
\mccorrect{NICK: As for the corresponding section above, aim to shorten to a single paragraph summarising key trends, e.g., by discussing in terms of key similarities and differences relative to the Dots data.}
...
\mccorrect{NICK: As above, I suggest editing here to include key numbers in the main text and cut the figures, and would recommend moving it (as a couple of summary sentences, capturing the key points) to the Methods section.}
The advice is generated probabilistically from the rules described previously in Table \@ref(tab:ac-acc-dots-m-profiles).
It is thus important to get a sense of the actual advice experienced by the participants.
...
\mccorrect{NICK: As I mentioned in our meeting just now, I was very surprised at this point to find out there were separate feedback / no feedback conditions. This needs much clearer signposting: In the introductory section, tell the reader that Dots experiments were (always?) run without feedback (and say why -- because we felt we knew what to expect from previous studies?), whereas Dates task studies had separate Feedback and No Feedback conditions (always? Make explicit that it's a between participant...
...manipulation, and remind the reader as needed (in the Methods: specify how many ppts were in each group, and how they were assigned, refer to "participants in the Feedback / No Feedback conditions" etc.). For the results here, make explicit when you're collapsing across conditions (in your analyses of basic accuracy, confidence, etc.) and explain why you're doing it (presumably: we don't expect the results to differ, find that they don't [not reported here] and for simplicity present the...
...pooled results where the differences are interesting/important.}
...
\mccorrect{NICK: Signpost as the key analysis.}
...
\mccorrect{NICK: Is there a difference across conditions? Interaction?}

\mccorrect{NICK: Scope to explore these results further: Does the preference correlate with actual accuracy/agreement differences as experienced across participants?}
