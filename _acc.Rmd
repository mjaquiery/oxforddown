...
In line with the preregistration, participants' data were excluded from analysis where they have an average accuracy below `r accuracyRange[1]` or above `r accuracyRange[2]`, do not have choice trials in all confidence categories (bottom 30%, middle 40%, and top 30% of prior confidence responses), have fewer than `r minTrialsPerCategory` trials in each confidence category, or have completed the experiment after the preregistered amount of data has already been collected. 
...
\mccorrect{NICK:Also, not to change now, but to flag for the viva: The figure brings out some of the questionable features of how you "cap" influence. It doesn't make sense to me that the maximum confidence change for disagreement with high initial confidence is small. I realise why you do it mathematically, but it's very odd conceptually when these are arguably the most influential changes -- when a person shifts their view initially held with near certainty. MATT: Should we keep the off-diagonal figure thing to illustrate this?}
...
agr.table %>%
  ...
  kable(label = "\\label{tab:ac-acc-dots-r-advice-agr}Advisor agreement for Dots task Accuracy experiment")
...
agr.table %>% 
  ...
  kable(label = "\\label{tab:ac-acc-dots-r-advice-acc}Advisor accuracy for Dots task Accuracy experiment")
...
The advisors' actual accuracies matched the target accuracies (Table \@ref(tab:ac-acc-dots-r-advice-acc)), with `r n_ap`/`r length(unique(familiarization$pid))` participants experiencing the planned relationship wherein the High accuracy advisor's advice was more accurate than the Low accuracy advisor's advice.
...
\mccorrect{TODO: Do we want to perhaps look at the last half of the trials on these blocks for influence, so we can make the argument that they've had a chance to get to know the advisors a little? How long are the blocks?}
...
This experiment \mccorrect{NICK:provided a basic exploration of advice seeking and advice use in the Dates Task (if it's the first one reported), and aimed to replicate the results of Experiment X in the context of this task.}
...
This study used the binary version of the Dates Task (§\@ref(m-p-dates-b)) in which \mccorrect{NICK:XX} participants \mccorrect{NICK:did XYZ. Helpful here to give a brief reminder in a short sentence. Also: XX blocks of YY trials in which they chose between advisors on each trial, with ZZ trials beforehand of forced advice to familiarise them with the advisors.}.
...
\mccorrect{NICK: Why 59% and not 60% as in the previous experiment? Isn't this number hard to fix with a reasonable number of trials? (vs. 6/10 agree when correct, 4/10 agree when incorrect). MATT: I've looked at this - it really was 59% but I can't remember why; presumably to balance agreement rates a bit given average participant accuracy but I'm not sure.}
...
\mccorrect{NICK: Maybe give the overall % of trials excluded on this basis.}
...
\mccorrect{NICK: This table feels a bit pointless given that nobody was excluded. Cut? (I guess it's there algorithmically because of the markdown format.)}
...
\mccorrect{NICK: This doesn't seem to match what's in the figure -- only an improvement with the good advisor, it looks to me.
Test of interaction?}
...
\mccorrect{NICK: I'd suggest just reporting the correlation values and stats in the main text, to reduce the number of figures.}
...
\mccorrect{NICK: As above, I'd suggest editing here to include key numbers in the main text and cut the figures, and would recommend moving it (as a couple of summary sentences, capturing the key points) to the Methods section.}
The advice is generated probabilistically from the rules described previously in Table \@ref(tab:ac-acc-dots-m-profiles).
It is thus important to get a sense of the actual advice experienced by the participants.
...
\mccorrect{NICK: I'd suggest framing along the following lines: This study has mixed results in relation to original predictions. With feedback, people showed a clear preference for the accurate advisor. Without feedback, this wasn't seen -- in contrast to Exp 1. But note: fewer trials, and also much more varied agreement rates in Exp 2.
