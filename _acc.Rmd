...
Participants started with `r sum(tmp$type == "practice")` blocks of `r tmp %>% filter(type == "practice") %>% pull(n) %>% round()` trials that contained no advice to allow them to familiarise themselves with the task.
...
agr.table <- tribble(
  ~`Advisor`, ~`Participant correct`, ~`Participant incorrect`, ~ `Overall`, ~`Overall accuracy`,
  "High accuracy", agr[1, 1], agr[2, 1], agr[1,1]*.5 + (1 - agr[1,1])*.5, agr[1, 1],
  "Low accuracy", agr[1, 2], agr[2, 2], agr[1,2]*.5 + (1 - agr[1,2])*.5, agr[1, 2]
) %>% 
  rename_with(~ paste0(., footnote_marker_alphabet(1)), Overall) %>% 
  prop2str()
...
generalTestBF(formula = pPickHigh ~ Accuracy * Feedback + pid, data = mutate(tmp, pid = factor(pid)), whichRandom = "pid", whichModels = "top")

lm_agr <- lm(
  formula = pPickHigh ~ Agreement * Feedback,
  data = tmp
)
generalTestBF(formula = pPickHigh ~ Agreement * Feedback + pid, data = mutate(tmp, pid = factor(pid)), whichRandom = "pid", whichModels = "top")
...
kableLM(
  lm_acc,
  caption = "\\label{tab:ac-acc-dates-r-cors-acc}Experienced accuracy difference effects in the Dates task with in/accurate advisors"
)
```
...
kableLM(
  lm_agr,
  caption = "\\label{tab:ac-acc-dates-r-cors-agr}Experienced agreement difference effects in the Dates task with in/accurate advisors"
)
```
...
Linear models were run predicting advisor selection behaviour based on experienced differences in accuracy (Table \@ref(tab:ac-acc-dates-r-cors-acc)) and agreement (Table \@ref(tab:ac-acc-dates-r-cors-agr)).
These models were complex to interpret because the results were not clearly significant, 
Despite this difference, there was no evidence of a relationship between participants' advisor preference and their experience of either advisor accuracy ($BF_{10}=$ `r bf2str(exp(r_acc@bayesFactor$bf))`) or advisor agreement ($BF_{10}=$ `r bf2str(exp(r_agr@bayesFactor$bf))`), with the latter indicating an absence of a relationship. 
This is not entirely surprising because, as with the accuracy correlations discussed above, there was relatively little variation in experience of advisors so effects might be expected to be small and difficult to detect.

### Discussion {#ac-acc-d}

\mccorrect{NICK: I suggest framing along the following lines: This study has mixed results in relation to original predictions. With feedback, people showed a clear preference for the accurate advisor. Without feedback, this was not seen -- in contrast to Exp 1. But note: fewer trials, and also much more varied agreement rates in Exp 2.
Might also be worth noting how much pick rates varied again -- but particularly in the No Feedback case. Early highlighting of a theme of the results -> when there's no overall preference at the group level, the pattern seems to be wide variation in pick rate (rather than pick rates clustering around 0.5).}
