...
Participants were systematically more confident on correct as compared to incorrect trials for both initial estimates and final decisions (Figure \@ref(fig:ac-acc-dots-r-performance)B).
There is considerable variation between participants both on their baseline confidence and on their variability (not shown).

ANOVA of Time, Advisor, and Correctness of the decision indicated that there was a main effect of Correctness, with participants being more confident on correct vs incorrect decisions (`r tmp$s[[3]]`), and that this effect showed interactions with Advisor (`r tmp$s[[5]]`) and Time (`r tmp$s[[6]]`), summed up in the three-way Advisor-Correctness-Time interaction (`r tmp$s[[7]]`) that indicated that confidence increased for correct responses and decreased for incorrect responses, and that this confidence change difference was greater for the High accuracy advisor (`r m$Split[2]`) than the Low accuracy advisor (`r m$Split[1]`).

###### Metacognitive ability

Where performance on the underlying task is held constant, as here at least for participants' initial pre-advice decisions, metacognitive sensitivity can be measured in a bias-free way by plotting Receiver Operating Characteristic (ROC) curves for metacognitive responses [@flemingHowMeasureMetacognition2014].
...
This study used the binary version of the Dates Task (§\@ref(m-p-dates-b)) in which \mccorrect{NICK:XX} participants \mccorrect{NICK:did XYZ}.
