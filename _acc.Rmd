...
### Experiment 1A: Dots Task {#ac-acc-dots}
...
\mccorrect{NICK:Include participant numbers; include number of trials, blocks, etc. and how they were organised. Any forced advisor trials in the main experiemnt? Even though it's in the title able "Dots task", it's helpful for the reader to include this detail in the text as a brief reminder: e.g. "Participants performed the dot count perceptual decision task in XX blocks of YY trials, with the choice between two advisors on each trial after their initial decision". Useful to make explicit (here or in the general methods) what the participants were told about advisors, if anything. E.g. were they told that the advisors would differ and it was up to them (the participants) to figure out whose advice they wanted? How many familiarisation trials etc.}
...
Despite this difference, there was no evidence of a relationship between participants' advisor preference and their experience of either advisor accuracy ($BF_{10}=$ `r bf2str(exp(r_acc@bayesFactor$bf))`) or advisor agreement ($BF_{10}=$ `r bf2str(exp(r_agr@bayesFactor$bf))`).
...
\mccorrect{NICK:Add a brief summary/discussion here. Note key results, and end with statement that we were next interested if we'd see the same result in a different task context.}

### Experiment 1B: Dates Task {#ac-acc-dates}
