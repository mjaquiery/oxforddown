...
In line with the preregistration, participants' data were excluded from analysis where they had an average accuracy below `r accuracyRange[1]` or above `r accuracyRange[2]`, do not have choice trials in all confidence categories (bottom 30%, middle 40%, and top 30% of prior confidence responses), have fewer than `r minTrialsPerCategory` trials in each confidence category, or have completed the experiment after the preregistered amount of data has already been collected. 
...
This is supported statistically by an ANOVA of response accuracy by Advisor and Time: there was no discernible effect of Advisor (`r tmp$s[[1]]`), but there was an interaction between Advisor and Time (`r tmp$s[[3]]`); and there was an effect of Time `r tmp$s[[2]]`. 
...
tmp <- familiarization %>% 
  mutate(agree = adviceSide == initialAnswer) %>%
...
mmAgree <- familiarization %>%
  mutate(agree = if_else(adviceSide == initialAnswer, "Agree", "Disagree")) %>%
  group_by(pid, agree) %>%
  summarise(influence = mean(adviceInfluenceCapped), .groups = "drop") %>%
  group_by(agree) %>%
  summarise(
    s = md.mean(influence, label = glue("M~{agree}~")),
    .groups = "drop"
  ) %>%
  unique()
...
This influence of advice is stronger for disagreeing than agreeing trials (even when corrected to account for the scale assymetry; `r aov_inf$s[2]`; `r mmAgree %>% pull(s) %>% paste(collapse = ", ")`).
This difference did not appear to be different between advisors (`r aov_inf$s[1]`), and as shown in Figure \@ref(fig:ac-acc-dots-r-performance)E, however, the level of influence was equivalent between advisors, with most participants being almost exactly equally influenced by both advisors (`r aov_inf$s[3]`).
...
mmTime <- Familiarization %>% 
  rename(
    Initial = responseAnswerSideCorrect, 
    Final = responseAnswerSideCorrectFinal
  ) %>%
  pivot_longer(c(Initial, Final)) %>%
  group_by(name, pid) %>%
  summarise(Accuracy = mean(value), .groups = "drop") %>% 
  group_by(name) %>%
  summarise(
    s = md.mean(Accuracy, label = glue("M~{name}~"), isProportion = T), 
    .groups = "drop"
  ) %>%
  unique()

mmInt <- Familiarization %>% 
  group_by(pid, Advisor) %>%
  summarise(
    across(starts_with("responseAnswerSideCorrect"), mean),
    .groups = "drop"
  ) %>%
  mutate(
    improvement = responseAnswerSideCorrectFinal - responseAnswerSideCorrect
  ) %>%
  group_by(Advisor) %>%
  summarise(
    s = md.mean(improvement, label = glue("M~Improvement|{Advisor}~"), isProportion = T),
    .groups = "drop"
  ) %>%
  unique()
...
Figure \@ref(fig:ac-acc-dates-r-performance)A also shows that participants managed to improve their performance from their initial estimates to their final decisions with both advisors (`r aov_acc$s[2]`; `r mmTime %>% pull(s) %>% paste(collapse = ", ")`). 
...
As would be expected from participants following advice, the improvement in accuracy from initial estimates to final decisions was greater for the High accuracy advisor than the Low accuracy advisor (`r aov_acc$s[3]`; `r mmInt %>% pull(s) %>% paste(collapse = ", ")`).
...
mmTime <- Familiarization %>% 
  rename(
    Initial = responseConfidence, 
    Final = responseConfidenceFinal
  ) %>%
  pivot_longer(c(Initial, Final)) %>%
  group_by(name, pid) %>%
  summarise(Confidence = mean(value), .groups = "drop") %>% 
  group_by(name) %>%
  summarise(
    s = md.mean(Confidence, label = glue("M~{name}~"), decimals = 1), 
    .groups = "drop"
  ) %>%
  unique()

mmInitialCor <- Familiarization %>%
  pivot_longer(c(responseConfidence, responseConfidenceFinal)) %>%
  mutate(
    initialCor = if_else(responseAnswerSideCorrect, "Correct", "Incorrect")
  ) %>%
  group_by(pid, initialCor) %>%
  summarise(value = mean(value), .groups = "drop") %>%
  group_by(initialCor) %>% 
  summarise(
    s = md.mean(value, label = glue("M~{initialCor}~"), decimals = 1),
    .groups = "drop"
  ) %>%
  unique()
...
mmInt <- Familiarization %>% 
  ...
  group_by(pid, initialCor) %>%
  summarise(
    across(starts_with("responseConfidence"), mean),
    .groups = "drop"
  ) %>%
  mutate(
    increase = responseConfidenceFinal - responseConfidence
  ) %>%
  group_by(initialCor) %>%
  summarise(
    s = md.mean(increase, label = glue("M~Increase|{initialCor}~"), decimals = 1),
    .groups = "drop"
  ) %>%
  unique()
...
Participants' initial estimates and final decisions were both systematically more confident when the initial estimate was correct as compared to incorrect (`r aov_conf$s[1]`; `r mmInitialCor %>% pull(s) %>% paste(collapse = ", ")`).
Participants were also more confident on final decisions than on initial estimates (`r aov_conf$s[2]`; `r mmTime %>% pull(s) %>% paste(collapse = ", ")`), and the increase over time only appeared for the trials where the initial estimate was correct (`r aov_conf$s[3]`; `r mmInt %>% pull(s) %>% paste(collapse = ", ")`).
...
inf_aov <- inf_aov$ANOVA %>%
  nest(d = -Effect) %>%
  mutate(s = map_chr(d, ~ glue(
    "F({.$DFn},{.$DFd})={num2str(.$F, 2)}, p={prop2str(.$p, minPrefix = '<')}"
  )))

mmAdv <- Familiarization %>% 
  group_by(pid, Advisor) %>%
  summarise(inf = mean(advisor0Influence), .groups = "drop") %>%
  group_by(Advisor) %>%
  summarise(
    s = md.mean(inf, label = glue("M~Influence|{Advisor}~"), decimals = 2),
    .groups = "drop"
  ) %>%
  unique()
...
mmInt <- Familiarization %>% 
  transmute(
    pid, Advisor, advisor0Influence, 
    feedback = if_else(feedback, "Feedback", "No feedback")
  ) %>%
  group_by(pid, Advisor, feedback) %>%
  summarise(influence = mean(advisor0Influence), .groups = "drop") %>%
  pivot_wider(names_from = Advisor, values_from = influence) %>%
  mutate(difference = `High accuracy` - `Low accuracy`) %>%
  group_by(pid, feedback) %>%
  summarise(difference = mean(difference), .groups = "drop") %>%
  group_by(feedback) %>%
  summarise(
    s = md.mean(difference, label = glue("M~Difference|{feedback}~"), decimals = 2),
    .groups = "drop"
  ) %>%
  unique()
...
The High accuracy advisor was substantially more influential than the other Low accuracy advisor (`r inf_aov$s[1]`, `r paste0(mmAdv$s, collapse = ", ")`).
This tendency did not differ significantly between the groups (`r inf_aov$s[3]`, `r paste0(mmInt$s, collapse = ", ")`), and nor did the participants in the feedback and no feedback conditions appear to differ in the extent to which they took advice (`r inf_aov$s[1]`).
