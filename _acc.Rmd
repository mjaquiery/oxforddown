...
tmp <- familiarization %>%
  ...
  summarise(
    ...
    `Final decision` = mean(confidenceShiftRaw),
    .groups = 'drop'
  ) %>%
  pivot_longer(cols = c(`Initial estimate`, `Final decision`),
...
m <- df %>% 
  ...
  pivot_wider(names_from = Correct, values_from = Difference) %>%
  mutate(Split = `TRUE` - `FALSE`) %>%
  select(Advisor, Split) %>%
  mutate(Split = num2str(Split))
