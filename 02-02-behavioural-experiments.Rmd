...
coefs <- cbind(
  ...
  mutate(
    ...
    s = glue(
      "{beta}={Estimate} [{`2.5 %`}, {`97.5 %`}], df={df}, p={`Pr(>|t|)`}"
    )
  )
...
bf <- tmp %>%
  ...
  select(pid, rpd, rd, A, D, I) %$%
  generalTestBF(
    I ~ A + D + rpd + rd + A:D + A:rpd + A:rd + D:rpd + D:rd + A:D:rpd + A:D:rd + pid, 
    data = .,
    whichRandom = "pid", 
    whichModels = "top",
    progress = F
...
A linear mixed model was used to better understand the significance of the relationships in the figure, with the form:
...
\beta 2 * \text{rpd}_t^a + 
\beta 3 * \text{rd}_t^a + 
\beta 4 * \text{rpd}_t^a * D_t^a +
\beta 5 * \text{rd}_t^a * D_t^a +
\epsilon_a + 
\epsilon_t
$$
where $I_t^a$ is the influence of trial $t$ for participant $a$, $D$ is the distance between the centre of the initial estimate and the advice for that participant on that trial, and $\text{rpd}$ and $\text{rd}$ are equal to 1 where that participant's rating of the advice on that trial was Possibly Dishonest or Dishonest, respectively. 
$\epsilon_a$ is the participant's intercept (a random factor in the model), and $\epsilon_t$ is the error between the predicted value and the observed value of influence.
