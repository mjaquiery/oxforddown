...
s_r <- sapply(1:length(r), \(i) paste(names(r)[i], num2str(r[i]))) %>%
  paste(collapse = ", ")
s <- glue("$\\chi ^2({chi$parameter}) = ${num2str(chi$statistic)}, p={prop2str(chi$p.value)}, BF={bf2str(exp(bf@bayesFactor$bf))}; AlwaysHonest:SometimesDeceptive ratio: {s_r}")

```

The advisors' advice was rated differently by the participants, as shown in Figure \@ref(fig:be-db-r-advice-ratings) (`r s`).
...
Familiarization %>%
  group_by(pid, advisor0idDescription, Rating) %>%
  summarise(woa = mean(advisor0WOA)) %>%
  ggplot(aes(x = Rating, y = woa)) +
...
x <- crossing(
  ...
  mutate(
    pHonest = map2_dbl(
      ...
      distance,
      ~ tmp %>%
        filter(pid == .x, Distance < .y) %>%
        pull(Honest) %>%
        mean()
    )
  )
thresh <- x %>%
  mutate(deviance = abs(2/3 - pHonest)) %>% 
  group_by(pid) %>% 
  filter(!is.nan(pHonest)) %>% 
  filter(deviance == min(deviance)) %>% 
  summarise(threshold = mean(distance))

tmp %>% 
  left_join(select(thresh, pid, threshold), by = "pid") %>%
  mutate(pid = fct_reorder(pid, threshold)) %>%
  ggplot(aes(y = pid, fill = Rating)) +
  geom_density_ridges(aes(x = Distance), alpha = 1/3, colour = NA) +
  facet_wrap(~Advisor) +
  theme(
