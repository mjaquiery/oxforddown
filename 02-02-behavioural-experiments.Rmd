...
In this experiment the effect of individuality is confounded with familiarity, a feature that we expect will increase the paletability of advice. 
...
Accuracy during the Familiarization trials was higher for final decisions than for initial estimates (`r s_acc$s[2]`), and higher for the Consistent individual advisor than for the group (`r s_acc$s[1])`.
There was no significant effect of advisor presentation on improvement (`r s_acc$s[3]`).

There were no significant effects for the ANOVA on confidence.
...
Likewise, no significant effects were found for the ANOVA on influence.
...
```{r be-ck-m-scorecard, fig.caption="Advice scorecard for Experimetn 7.  The scorecards indicated the relationship between confidence and accuracy, allowing participants to see how well calibrated they or an advisor was."}
...
Participants increased their accuracy from initial estimate to final decision (`r s_acc$s[2]`). There was no evidence of a difference between the advisors in either overall accuracy (`r s_acc$s[1]`) or in the increase in accuracy following advice (`r s_acc$s[3]`).

Participants were more confident in their responses when their initial estimate was correct (`r s_conf$s[1]`).
...
These predictions were not bourne out in the data. 
