...
old.trials <- read.csv('_old-influence-anova.csv') %>% mutate(across(c(pid, agree, hasChoice, advisor), factor))
tmp <- trials %>% arrange(pid, id) %>% filter(advisorId %in% 1:2) %>% transmute(pid = factor(pid), agree = factor(agree), hasChoice = factor((choice1 + choice2) == 3), advisor = factor(advisorId), adviceInfluence = round(adviceInfluence * 55), initialAnswer, finalAnswer, initialConfidence, finalConfidence, adviceInfluenceCapped = round(adviceInfluenceCapped * 55)) %>% arrange(pid, adviceInfluence)
tmp %>% View()

old.trials <- old.trials %>% arrange(pid, inf) %>% select(-pid)
old.trials %>% select(-pid)

a <- apply(tmp, 1, \(x) paste0(x, collapse = "|"))
b <- apply(old.trials, 1, \(x) paste0(x, collapse = "|"))

which(a %in% b)


ezANOVA(old.trials, dv = inf, wid = pid, within = c(agree, hasChoice, advisor))
ezANOVA(tmp, dv = adviceInfluence, wid = pid, within = c(agree, hasChoice, advisor))
...
tmp <- trials %>%
  filter(!is.na(Advisor), !is.na(confidenceCategory), !practice) %>%
  mutate(
    pid = factor(pid),
...
mmAdvisor <- trials %>%
  ...
  summarise(inf = mean(adviceInfluenceCapped), .groups = 'drop') %>%
  group_by(Advisor) %>%
  summarise(inf = mean(inf), .groups = 'drop') %>%
  mutate(inf = num2str(inf))
...
As shown in Table \@ref(tab:ac-ex1-influence-t), the only statistically significant effect was the main effect of agreement, with disagreement producing higher influence than agreement \mccorrect{!TODO[marginal means]}.
