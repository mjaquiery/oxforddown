...
tmp <- trials %>%
  ...
  summarise(inf = mean(inf), .groups = 'drop')
...
No main effect was found for advisor (`r aov_inf$s[1]`, M~Bias-sharing~=`r mmAdvisor %>% filter(Advisor == "Bias-sharing") %>% pull(inf)`, M~Anti-bias~=`r mmAdvisor %>% filter(Advisor == "Anti-bias") %>% pull(inf)`), meaning that the previous finding was not replicated.
As shown in Table \@ref(tab:ac-ex1-influence-t), the only statistically significant effect was the main effect of agreement, with disagreement producing higher influence than agreement (M~).
