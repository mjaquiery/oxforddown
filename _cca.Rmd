...
\mccorrect{NICK: Things got a bit confusing from here. I didn't really understand why you present the online replication study before the lab study, and I struggled to follow the narrative. I'm guessing this is a combination of two separate bits of work -- a description of the online studies that follows the structure and format of the earlier ones in this chapter, and then text from your transfer report for the in-lab study. I think it needs work to streamline and harmonise before I can provide useful comments. My overall suggestions would be:
1- Present the studies in the opposite order
2- Start with the in-lab study which has some suggestive results: significant effect where we originally planned to look, in medium confidence trials, but not convincing and not really obvious why the preference shouldn't be there for other confidence levels. For this study, consider which analyses you really need to tell the story. The aim here in the thesis is different to the transfer report / earlier write up. There's quite a bit of exploratory analysis that was useful at the time in terms of us understanding the data, but that doesn't really fit the narrative focus now developed in the rest of this chapter.
3- Then the online replication: can we find evidence in a larger online sample. (no)
4- Discussion: no confidence-weighted effect? Is it plausible people could learn it in real life? Or are pick rates too variable to reveal subtle differences in advisor preference. (The latter is consistent with the high variability we see -- the preferences have to be quite strong at the group level to see anything)}

### Lab study of confidence-contingent advice {#ac-cca-lab}

@pescetelliRoleDecisionConfidence2021 showed that, in the absence of objective feedback, advice was more influential coming from an advisor who agrees with a participant when that participant is confident (_Bias Sharing_) than coming from an advisor who agrees with a participant when that participant is unconfident (_Anti Bias_).
...
@pescetelliRoleDecisionConfidence2021used a judge-advisor system to demonstrate that judges are influenced to a greater extent by advisers who share their biases.
...
\mccorrect{NICK: Here give a brief overview: Here advice was carefully controlled as a function of participants' initial decisions and confidence. Following P&Y's design, their confidence on each trial was assessed (as being low/med/hi for them). One advisor agreed more often when participants expressed high confidence than low; the other did the opposite; agreement was matched for medium confidence.}
