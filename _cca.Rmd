...
On some trials participants were able to choose which of their advisors would provide the advice (Figure \@ref(fig:ex1-paradigm)).
...
Overall, `r nrow(participants)` participants (M~age~ = `r num2str(mean(participants$age))` ±SD `r num2str(sd(participants$age), 1)`, `r sum(tolower(participants$gender) == 'm')` male, `r sum(tolower(participants$gender) == 'f')` female, `r sum(tolower(participants$gender) != 'm' & tolower(participants$gender) != 'f')` other) recruited from the University of Oxford participant recruitment platforms took part in the experiment.
An additional 2 participants attended experimental sessions but their data were not analysed because they were in excess of the number stated in preregistration.
...
trials <- trials %>% 
  ...
  mutate( 
    ...
    initialConfidence = cj1, finalConfidence = cj2,
    initialAnswerCorrect = cor1, finalAnswerCorrect = cor,
    initialAnswer = int1, finalAnswer = int2,
    correctAnswer = wherelarger,
  ...
  select(
    pid, id, block, feedback, obsacc, agree, step, advisorId, Advisor,
    matches('choice[12]'), matches('^(initial|final)'),
    confidenceCategory, correctAnswer = wherelarger, contains('time')
  )
...
The Anti-bias advisor agrees more frequently with participants when their initial estimate is correct and made with relatively low confidence (Table \@ref(tab:ex1-advisor-profiles)).
...
\mccorrect{TODO: This table looks sensible, but has the same variable across 2 different columns. Reformat it so we can tie it neatly to the observed values in the Results.}
...
kable(ex1.table, caption = "\\label{tab:ex1-advisor-profiles}Experiment 1 advisor advice profiles") %>%
  kable_styling() %>%
...
bind_rows(
  ...
  # Incorrect trials
  trials %>% 
    filter(!as.logical(initialAnswerCorrect), !is.na(Advisor)) %>%
  ...
  mutate(
    ` ` = case_when(
      initialAnswerCorrect == 1 ~ "Participant correct",
      initialAnswerCorrect == 0 ~ "Participant incorrect",
      T ~ "Total agreement"
    ),
    agr = prop2str(agr)
  ) %>%
