...
simulate_participant <- function(
  ...
  for (t in out$trialId) {
    ...
    # Confidence shift
    advisorTrust <- ifelse(
      out$advisorIndex[t] == 1, 
      out$advisorTrust1[n + 1], 
      out$advisorTrust2[n + 1]
    )
    
    out$confidenceShift.z[t] = (out$advisorAgrees[t] - .5) * advisorTrust
  }
...
gridSearch <- function(x, grid = tidyr::crossing(a = seq(-5, 5, .05), b = seq(-1, 1, .05))) {
...
visualiseRun <- function(x) {
  d <- x  %>%
    ...
    mutate(selected = advisorIndex == advisor)
  ggplot(d, aes(x = trial, y = trust, linetype = advisor)) +
    geom_col(
      aes(y = confidenceShift.z, fill = advisorAgrees), 
      alpha = .5, 
...
visualiseGrid <- function(gs, target_value = NULL) {
  gs <- gs %>%
    rename(ws_err = weightedSelection_error, tu_err = trustUpdateRate_error) %>%
    mutate(error = scaledError(ws_err, tu_err))
...
gd <- sim %>%
  mutate(uid = 'sim') %>%
  nest(data = -uid) %>%
  crossing(tibble(run = 1:100)) %>%
  mutate(gd = map(data, ~ gradientDescent(.) %>% gradientDescentSummary())) %>%
  unnest(cols = gd)
...
gs <- sim %>%
  mutate(uid = 'sim') %>%
  nest(data = -uid) %>%
  gridSearch(grid = tidyr::crossing(a = seq(-1, 5, .01), b = seq(-.5, 1.5, .01)))
Sys.time() - t1
...
gs <- sim %>%
  mutate(uid = 'sim') %>%
  nest(data = -uid) %>%
  gridSearch(grid = tidyr::crossing(a = seq(-1, 5, .005), b = seq(-.5, 1.5, .01)))
Sys.time() - t1
...
sim <- tibble(
  ...
  choice1 = 2,
  advisorAgrees = T,
  confidenceShift.z = c(.1, .15, .2, .25, .3, .1, .15, .35, .4, .45)
) %>% 
  nest(data = -uid) %>%
  crossing(tibble(run = 1:100))
...
simulants <- parameters %>%
  nest(params = -uid) %>%
  mutate(data = map(params, simulate_participant)) %>%
  unnest(cols = params)
...
for (s in 1:nrow(simulants)) {
  for (type in c('simulants', 'shuffled_simulants')) {
    ...
    print(ggplot(gd, aes(x = tu_end, y = ws_end, fill = tu_error + ws_error)) +
            geom_point(size = 4, alpha = .25) +
            geom_point(size = 4, colour = 'red', data = best) +
            annotate(geom = 'point', shape = 8, size = 4, colour = 'black', x = best$trustUpdateRate[1], y = best$weightedSelection[1]) +
            labs(
              caption = glue('Target: tu = {round(best$trustUpdateRate[1], 4)}, ws = {round(best$weightedSelection[1], 4)}'),
              title = type
            ))
  }
...
## Parameter recovery with 10 trials

```{r short parameter recovery setup}

simulants.short <- simulants %>% 
  mutate(data = map(data, ~ .[1:30, ]))
shuffled_simulants.short <- simulants.short %>%
  mutate(data = map(data, ~ mutate(., advisorAgrees = sample(.$advisorAgrees))))
```

```{r short visualise simulants data}

for (i in 1:nrow(simulants.short)) {
  print(visualiseRun(simulants.short$data[[i]]) + labs(title = 'sim'))
  print(visualiseRun(shuffled_simulants.short$data[[i]]) + labs(title = 'sim_shuffled'))
}

```

```{r short visualise simulant grid example}
t1 <- Sys.time()

for (i in 1:nrow(simulants.short)) {
  print(visualiseGrid(
    gridSearch(simulants.short[i, ]), 
    target_value = simulants.short[i, c('trustUpdateRate', 'weightedSelection')]
  ) + labs(title = 'sim'))
  print(visualiseGrid(
    gridSearch(shuffled_simulants.short[i, ]), 
    target_value = simulants.short[i, c('trustUpdateRate', 'weightedSelection')]
  ) + labs(title = 'sim_suffle'))
}
...
```{r short parameter recovery demo}
...
for (s in 1:nrow(simulants.short)) {
  for (type in c('simulants.short', 'shuffled_simulants.short')) {
    sim <- get(type)[s,  ] %>%
      crossing(tibble(run = 1:100))
...
```{r short parameter recovery demo evaluation}
