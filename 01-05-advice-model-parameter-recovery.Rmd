...
shuffled_simulants <- simulants %>% 
  mutate(data = map(data, slice_sample, prop = 1))
```
...
for (s in 1:nrow(simulants)) {
  ...
  print(ggplot(gd, aes(x = tu_end, y = ws_end, fill = tu_error + ws_error)) +
          ...
          labs(caption = glue('Target: tu = {round(best$trustUpdateRate[1], 4)}, ws = {round(best$weightedSelection[1], 4)}')))
}
