...
for (i in 1:nrow(sims)) {
  print(visualiseGrid(
    gridSearch(sims[i, ], grid = crossing(a = seq(-10, 30, .5),
                                          b = seq(-.1, 1.5, .1))), 
    target_value = sims[i, c('trustUpdateRate', 'weightedSelection')]
  ) + labs(title = 'sim'))
  print(visualiseGrid(
    gridSearch(shuffled_sims[i, ], grid = crossing(a = seq(-10, 30, .5),
                                          b = seq(-.1, 1.5, .1))),
    target_value = sims[i, c('trustUpdateRate', 'weightedSelection')]
  ) + labs(title = 'sim_suffle'))
}
...
for (i in 1:nrow(sims)) {
  x <- gridSearch(sims[i,], grid = crossing(a = seq(-15, 30, .5), b = fixed_TUR))
