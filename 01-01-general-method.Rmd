...
The specific number of practice trials differed between experiments as we sought to balance minimising the participant time requirement with minimising practice effects.
...
The Dates task presented participants with historical events that occurred in a specific year in the 20th century. 
...
Advisors in these experiments typically placed their advice markers by identifying a target year, either the correct year or the centre of the participant's initial estimate marker, and then placing the centre of their advice marker on a year sampled from a normal distribution around that target year.
On occasion, advisors would offer 'off-brand' advice, designed to neither be close to the participant's initial estimate nor the correct year. 
...
Where advisors gave estimates with a confidence, they first calculated an internal estimate of when they believed the event took place by drawing from a normal distribution around the correct year.
They then determined their answer and their confidence by comparing their belief about the year the event took place with the anchor year, and scaling their confidence as a function of the distance between those years.
...
\mccorrect{QUESTION: I'm pretty sure I can put my hands on the experimental code, screenshots of the estimation task, and the brief write-ups or analysis of the responses. Likely I have the raw data for the responses, too. Is it worth including that stuff, either here in brief or in an Appendix?}
...
Another major difference between real-life advice and the advice offered in these experiments is in the richness of the relationship between advisor and judge.
...
The data collected during these runs is available alongside the data collected in the final versions of experiments.
...
Pilot data was collected to ensure the study functioned properly, and so the data are not analysed with regard to the hypotheses. 
