...
# For some reason they only work one-by-one
knit: (function(input, ...) {
    thesis_formats <- "bs4"
    
    source("scripts_and_filters/knit-functions.R");
    knit_thesis(input, thesis_formats, envir = .GlobalEnv, ...)
  })
...
#  
description: |
  This Experimental Psychology DPhil thesis for the University of Oxford 
  explores how experience of advice changes estimates of advisor reliability 
  with and without feedback. 
  It also investigates whether egocentric discounting is explicable as a 
  rational response to basic uncertainties of information transmission.
...
dedication: For Mary Ann Furze, who would have got one too if it weren't for the meddling patriarchy.
...
use-biblatex: true
bib-latex-options: "style=authoryear, sorting=nyt, backend=biber, useprefix, doi=true, isbn=false, maxbibnames=100, uniquename=false" #for science, you might want style=numeric-comp, sorting=none for numerical in-text citation with references in order of appearance
...
output:
  ...
  bookdown::bs4_book: 
    ...
    theme:
      primary: "#6D1919"
    repo: https://github.com/mjaquiery/oxforddown
    pandoc_args: "--lua-filter=scripts_and_filters/colour_and_highlight.lua"
  bookdown::gitbook:
...
```{r setup-main, echo = F, include = F}
# When knitting, do the whole shebang
options(
  # Setting ESM.recalculate to T or a positive integer will recreate cached 
  # files. This can be very time-consuming.
  # F ~ 10min
  # 1 ~ 1h
  # 2 ~ 6h
  ESM.recalculate = F
