...
agr.table %>%
  ...
  kable(caption = "\\label{tab:ac-agr-dots-r-advice-agr-t}Advisor agreement for Dots task Accuracy vs Agreement experiment")
...
agr.table %>% 
  ...
  kable(caption = "\\label{tab:ac-acc-dots-r-advice-acc-t}Advisor accuracy for Dots task Accuracy vs Agreement experiment")
...
\mccorrect{NICK: Add a quick intro -- at least to make clear that this exp compares Feedback and No Feedback conditions, but otherwise looks at the same Q again. Probably worth flagging up the switch the continuous version of the task, and give the rationale for this.}
...
\mccorrect{NICK: given the shift in method, you need to remind the reader about how the continuous version works.
Including how confidence was rated, how agreement is defined, etc., and how feedback was provided.}
...
\mccorrect{NICK: Crucially, in the same way for both conditions: So we could look at the impact of advice from the two advisors while avoiding the confounds inherent in their usual advice patterns.}

\mccorrect{NICK: Explain in detail how the advice from each advisor was constructed.}
...
Figure \@ref(fig:ac-ava-dates-r-advice-agr) shows the agreement rates experienced by each participant, where agreement is defined in terms of whether or not the advice marker overlaps with the participant's original estimate. 
There was a mixture of participants who experienced a higher agreement rate each advisor.
\mccorrect{NICK: I suppose this is technically correct, but from the figure it looks like there was only one anomolous participant.}
...
Consistent with the result from the Dots task \mccorrect{NICK: Experiment XX}, in the No feedback condition participants' preferences for receiving advice from the High accuracy advisor were not different from chance (`r .T.Nfb`) on average, and varied widely across individual participants: Participant preferences in the No feedback condition were almost perfectly evenly distributed, both in terms of which advisor was preferred and the strength of that preference, in both the original study and the replication (Figure \@ref(fig:ac-ava-dates-r-graph)).
...
\mccorrect{NICK: Refer explicitly to the significant interaction.}
