...
\mccorrect{NICK: Reading on, we don't contrast feedback and no-feedback cases in the Dots task, so this text needs revising to reflect this. Here, much more than in previous experiments, it's a little awkward that we don't have a feedback version of the dots task.}
...
\mccorrect{NICK: Comment here in the text on how the advisors were constructed -- don't rely on the reader to figure out the key points from the table alone.}
...
\mccorrect{NICK: Much higher numbers than in previous experiments. Any reason why?}
...
\mccorrect{NICK: Summarise this section in a single focused paragraph, cutting RTs and giving descriptive stats and test results in-line, cutting most of the figures.
One option would be to retain the results presentation and figures for accuracy and confidence (i.e., showing objective performance benefits from being paired with the accurate advisor), and cut the rest. I don't think accuracy, AUROC, etc. add much at this point. Could just say that these basic performance metrics were very similar.}
...
\mccorrect{NICK: And also counter to our predictions...}
...
\mccorrect{NICK: Any other analyses to explore this (lack of) effect? E.g., how many participants fall outside some reasonable likelihood bound according to binomial distribution assuming random responding? Are the differences predicted at all by differences in  experienced accuracy/agreement? etc.}
