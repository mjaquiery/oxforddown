...
for (i in 1:ceiling(nrow(parameters) / nCores)) {
  ...
  fName <- glue('cache/network-models/100a2000d_{n[1]}-{n[2]}.rda')
...
t1 <- Sys.time()
fName <- 'cache/network-models.rda'
if ((is.null(getOption('ESM.recalculate')) || getOption('ESM.recalculate') < 2) 
    && file.exists(fName)) {
  load(fName)
} else {
...
for (i in 1:ceiling(nrow(parameters.emp) / nCores)) {
  ...
  fName <- glue('cache/network-models/emp/100a{nD}d_{n[1]}-{n[2]}.rda')
  if ((is.null(getOption('ESM.recalculate')) || getOption('ESM.recalculate') < 2) 
      && file.exists(fName)) {
    next()
  }
...
t1 <- Sys.time()
fName <- 'cache/network-models-emp.rda'
if ((is.null(getOption('ESM.recalculate')) || getOption('ESM.recalculate') < 2) 
    && file.exists(fName)) {
  load(fName)
} else {
...
agents.all %>%
  nest(d = -c(trust_weights, weighted_sampling, model_num, burnIn)) %>%
  mutate(cors = map(
    model_num, 
    function(.x) {
      gs <- graphs.all[[.x]]
  ...
  geom_point(aes(y = r, colour = p < .05)) +
