...
To account for different distributions of errors across each dimension, the overall model error is calculated by normalising each error dimension seperately and then summing them.
...
In both cases, the mean trust update parameter is just above 0 (overall mean = `round(mean(coefs$trustUpdateRate), 3)`), as is the weighted selection parameter (overall mean = `round(mean(coefs$weightedSelection), 2)`). 
...
Verification of this model fitting approach was conducted on the Advanced Research Cluster \mccorrect{!TODO[CITATION_NEEDED]} in two ways. 
