...
Each model consists of a population of agents implementing the same mathematical model of decision-making, trust-updating, and advisor selection, with different coefficients for parameters of that mathematical model (Table \@ref(tab:nw-r-agent-properties-table)) drawn from appropriate distributions.
...
Each agent has the properties listed in Table \@ref(tab:nw-r-agent-properties-table).
An agent's values for each of these properties (except id) are created by drawing values from normal distributions defined by parameters in the model settings (Table \@ref(tab:nw-r-model-properties-table)).
...
tribble(
  ...
  kable(
    caption = "\\label{tab:nw-r-agent-properties-table}Agent properties",
    escape = F
  )
...
tribble(
  ...
  "\\texttt{truth\\_sd}", "Mean, standard deviation for true world values", "model",
  ...
  kable(
    caption = "\\label{tab:nw-r-model-properties-table}Model properties",
    escape = F
  )
...
\mccorrect{!TODO[Histogram of coefficients]}

\mccorrect{!TODO[Correlation of trust update and weighted selection]}
