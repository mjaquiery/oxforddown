...
The behavioural experiments in the previous chapter, along with predecessors from others in our lab [@pescetelliUseMetacognitiveSignals2017], indicate that people show preferences for selecting advisors who agree with them and treat advice which agrees with their initial judgements as more valuable. 
Previous modelling work has shown that either of these properties is sufficient to produce echo-chamber-like structures in networks wherein individuals increasingly disregard information from those likely to disagree with them \mccorrect{!TODO[citations]}.
...
The intuitive nature of its explanations means that it is easy to lose touch with ecological validity (because even non-ecologically valid implementations seem valid when expressed in agent-based terms) and easy to overclaiming on the basis of results.
...
Rather than attempt to present a full picture of agent-based models in which agents advise one another, this section briefly covers modles that include a tendency for agents to seek out advice from likeminded others and compare the effects of varying this property. 
...
In their model, agents make a single binary decision at each step on the basis of a perception of the true value and an idiosyncratic (static) bias.
Agents then consult another agent for advice, and update their final decision by integrating that advice with their own opinion, weighted by the trust they have in their advisor. 
...
The model used here is also underpinned by the update-from-information theoretical approach described above.
Thus, like in other models, agents interact with one another to share information, process that information differently depending on its source, and refine their opinion of the source from the information provided. 

In the current model, the agents face a perceptual decision in which they integrate sensory evidence with prior expectations in a Bayesian manner to produce an initial decision. 
...
The advice is integrated with the initial decision, again in a Bayesian manner, to produce a final decision. 
...
The initial decision -- advice -- final decision -- update cycle is repeated thousands of times.
The models illustrate they dynamics of advice interactions: we can observe how prior expectations change over time and whether and how polarisation or consensuses emerge; and how agents' trust in one another changes as they gain experience with one another.
...
The two free parameters estimated during the parameter recovery process are: how rapidly trust updates; and the extent to which more trusted advisors are preferred.
...
To allow the reader to determine whether the omissions are important, here is the list of omitted phenomena and features:
...
* Normalisation of trust weights or decay of trust over time  
    
#### Differences between experiments and models
* Picking between one and many is definitely not the same  
...
* Parameter space is huge and dynamics dependent upon parameter interactions  
  ...
  * This is what @bruchAgentBasedModelsEmpirical2015 call 'model uncertainty' and we did try to reduce it a bit by exploring different stuff in development, but ultimately it's a computational resources issue  
* Some models are pretty huge (n ~ 2000 is common). These are smaller to remain computationally tractable and because 100 is enough to see the phenomena we wish to model.  
  * It would be possible to extend the models, but the very large models have to run extremely slowly for computational tractability reasons (write to disk rather than storing in RAM; could be rewritten but n ~ 100 was part of the original design hence why they are how they are).  

* Basically, this is all illustration and you can't lean on it too heavily   
