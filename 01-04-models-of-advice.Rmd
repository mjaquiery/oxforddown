...
\mccorrect{!TODO[Graph with parameter recovery accuracy stuff]}
\mccorrect{!MAYBE[Figure showing the shuffling process?]}
...
\mccorrect{!TODO[Error distributions for real/shuffled data and shuffle positions]}
...
As with previous simulation-based approaches ([§Similar models in the literature](#ne-i-lit)), this work showed that echo chambers and polarisation can occur in networks of agents using heuristics that appear from the agent's perspective to be rational. 
...
We acknowledge that the specific values of parameters estimated here may not be a good description of people's tendencies in the kind of task simulated: because of differences between the binary advisor choice situation faced by the participants and the mulitple advisor choice situation faced by the agents; or because of difficulties in accurately estimating the coefficients for some participants; or because the the participant population may deviate somewhat from humanity in general.
...
Additionally, there may be variation in confidence across personality and psychiatric dimensions \mccorrect{QUESTION: Nick, do you know good citations for this?}, and computational models are increasingly used to explore the implications of these kind of variations in psychiatric disorders \mccorrect{QUESTION: Also a citation for this - Nick, do you know of one? I guess it's computational psychiatry in general?}.
...
Firstly, advice is the key feature of the model rather than the task, and so the agents' performance on the task does not vary over time (they don't get better or worse, and they don't get bored). 
...
While the models discussed in [§Similar models in the literature](#ne-i-lit) had smaller sizes, the largest being 1000 agents, it is not uncommon to see models with 2-3000 agents [@nowakPrivateAttitudePublic1990; @dugginsPsychologicallyMotivatedModelOpinion2017; @songDynamicSpiralsPut2017].
...
\mccorrect{NICK: Add a brief conclusion paragraph here in the spirit of "These limitations notwithstanding, the simulations show..."} Given this is the end of this line of work, it might make sense to consider the simulations together with the experiments in the previous chapter, along the lines of: Here I've used agent-based models to explore the implications of selective advice sampling. The experiments in Chapter 5 showed XYZ. The simulations here showed ABC.}
