...
Firstly, simulated participant datasets were generated by using the recovery model in reverse with know coefficients, and then these coefficients were recovered using the recovery procedure.
...
Secondly, error scores were compared for coefficients fitted to each participants data and coefficients fitted to versions of that participant's data where the advisor agreement column values had been shuffled. 
...
During development we observed that higher weighted selection values (around 10) tended to accelerate the polarisation of the model. 
