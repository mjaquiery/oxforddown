...
tmp <- slice_sample(d, n = 1) %>%
  unnest(MSE) %>%
  pivot_longer(
    matches('Model[0-9]+'), 
    names_to = 'model', 
    names_pattern = "Model([0-9]+)",
    values_to = 'MSE'
  ) %>%
  mutate(
...
tmp <- tmp %>% 
  ...
  filter(Advisor != 0)
tmp %>%
  ggplot(aes(x = trial_id, y = Trust)) +
  geom_rug(colour = 'grey', length = unit(.06, 'npc'), sides = 't', data = filter(tmp, !is.na(choice0))) +
  geom_rug(sides = 't', data = filter(tmp, advisorAgrees)) +
  geom_line() +
  geom_point(aes(colour = advisorAgrees)) +
  facet_wrap(~paste0('M', model, 'A', Advisor), scales = 'free_y')
