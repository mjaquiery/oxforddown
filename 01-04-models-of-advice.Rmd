...
for (f in list.files(
  "~/Downloads/thesis-parameter-estimation", 
  full.names = T,
  pattern = "\\.Rdata$"
)) {
  load(f, envir = tmpEnv)
...
positions %>%
  filter(d > 1) %>%
  mutate(across(ends_with("rank"), ~ . / d)) %>%
  pivot_longer(ends_with("rank"), names_to = "Dimension", values_to = "Rank") %>%
  ggplot(aes(x = Rank)) +
...
\mccorrect{TODO: Shuffles for all participants; use a joint error to calculate shuffle position.}
...
\mccorrect{!MAYBE[Other stuff of interest -- maybe we want to relate the variables to participants' ability to do the task or propensity to pick one advisor over another?]}
...
\mccorrect{!MAYBE[can we do a comparison of variability for participants who saw multiple pairs of advisors?]}
