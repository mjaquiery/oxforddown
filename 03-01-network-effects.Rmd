...
summary_fun <- function(m) {
  ...
  if (m$parameters$random_seed != 20210507) {
    m$model <- NULL
  }
...
runs <- output %>% 
  ...
  mutate(
    ...
    echoChamber_original = group_ratio_original > 1.05
  ) %>%
  select(
...
run_summary <- runs %>%
  group_by(
    ...
    confidence_weighted, biasUpdates, flatStart, bias_sd
  ) %>%
  summarise(
    across(.fns = list(mean = mean, sd = sd), .names = '{.col}_{.fn}'),
    .groups = 'drop'
  ) %>% 
  arrange(group_ratio_original_mean) %>%
  select(
    starts_with('echo'), 
    starts_with('group'), 
    everything(), 
    -matches('^echo.+_sd$', perl = T)
  )
