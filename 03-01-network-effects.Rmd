...
```{r network-models-increased-bias, eval=!(hasName(.Options, 'ESM.skip') && getOption('ESM.skip'))}
...
```{r network-models-one-group-accurate}
...
parameters <- data.frame(
  ...
  bias_sd = 1,
  sensitivity_sd = 1,
  trust_volatility_mean = c(.1, .1, .05, .05),
  trust_volatility_sd = c(.05, .05, .01, .01),
  bias_volatility_mean = 0,
  bias_volatility_sd = c(0, .05, 0, .05),
  starting_graph = starting_weight,
  randomSeed = 20201014
...
```{r network-models-weighted-selection}
...
```{r network-models-weighted-sampling-long, eval=!(hasName(.Options, 'ESM.skip') && getOption('ESM.skip'))}
