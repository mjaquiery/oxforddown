...
parameters <- tidyr::crossing(
  ...
  tidyr::crossing(
    ...
    weighted_sampling_mean = c(0, 10, 30)
  )
)
...
output <- models %>%
  ...
  bind_rows() %>%
  mutate(burnInPeriod = n_decisions - min(n_decisions))
...
for (v in names(select(output, burnInPeriod, bias_sd:starting_graph_type, truth_sd, confidence_weighted))) {
  ...
  print(
    output %>% 
      mutate(grp = glue(
        '{',
        paste0(names(select(output, burnInPeriod, bias_sd:random_seed, truth_sd:confidence_weighted, -!!sym(v))), collapse = "}|{"),
        '}'
      )) %>%
      rowid_to_column() %>%
      select(rowid, meanBiasMagnitude, cluster_count, group_ratio_empirical, burnInPeriod, bias_sd:random_seed, truth_sd:confidence_weighted, grp) %>%
      pivot_longer(c(meanBiasMagnitude, cluster_count, group_ratio_empirical), names_to = 'outcome') %>%
