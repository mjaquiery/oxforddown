...
for (i in 1:ceiling(nrow(parameters) / nCores)) {
  ...
  fName <- glue('cache/network-models-cuF/100a3000d_{n[1]}-{n[2]}.rda')
...
```{r basic models load}
t1 <- Sys.time()
fName <- 'cache/network-models-cuF.rda'
if ((is.null(getOption('ESM.recalculate')) || getOption('ESM.recalculate') < 2) 
    && file.exists(fName)) {
  load(fName)
} else {
  ...
  agents.cuF <- NULL
  graphs.cuF <- list()
  fList <- list.files('cache/network-models-cuF/', pattern = '', full.names = T)
  i <- 1
  for (f in fList) {
    load(f)
    for (m in models) {
...
The task the agents face is roughly similar to the task faced by human participants in the Dots Task experiments [§Perceptual decision (Dots Task)](#m-p-dots), where the participants make a decision about which of two rapidly and simultaneously presented grids contained more dots.
...
```{r model property table}

tribble(
  ~ `Property`, ~ `Description`, ~ `Type`,
  "$\\text{n_agents}$", "Number of agents to simulate", "model",
  "$\\text{n_steps}$", "Number of decisions to simulate", "model",
  "$\\text{decision_flags}$", "Binary flags indicating whether trust and/or bias update at each step", "model",
  "$\\text{feedback_probability}$", "Probability feedback is provided each step", "model",
  "$\\text{feedback_proportion}$", "Proportion of the population receiving feedback when provided", "model",
  "$\\text{random_seed}$", "Seed used for the pseudorandom number generator to allow repetition of runs", "model",
  "$\\text{truth_sd}$", "Mean, standard deviation for true world values", "model",
  "$\\text{confidence_weighted}$", "Whether agents use their own confidence to modify their trust updates", "model",
  ...
  kable(caption = "\\label{tab:model-properties}Model properties")
...
* $\text{decision\_flags}$, whether or not there is time for the random network weights to update before agents' biases begin to update
...
Each model was run `r length(unique(agents.cuF$random_seed))` times with different random seeds to check which features were consistent across runs. 
...
for (i in 1:ceiling(nrow(parameters.cuF.emp) / nCores)) {
  ...
  fName <- glue('cache/network-models-cuF-emp/100a3000d_{n[1]}-{n[2]}.rda')
  if ((is.null(getOption('ESM.recalculate')) || getOption('ESM.recalculate') < 2) 
      && file.exists(fName)) {
    next()
  }
...
t1 <- Sys.time()
fName <- 'cache/network-models-cuF-emp.rda'
if ((is.null(getOption('ESM.recalculate')) || getOption('ESM.recalculate') < 2) 
    && file.exists(fName)) {
  load(fName)
} else {
  ...
  agents.cuF.emp <- NULL
  graphs.cuF.emp <- list()
  fList <- list.files('cache/network-models-cuF-emp/', pattern = '', full.names = T)
  i <- 1
  for (f in fList) {
    load(f)
    for (m in models) {
