...
```{r}
cacheFile <- '_cache/strong-biases.rda'
...
```{r}
cacheFile <- '_cache/weighted-sampling.rda'
if (!getOption('ESM.recalculate') & file.exists(cacheFile)) {
  load(cacheFile)
} else {
  starting_weight <- .1
...
* $P(pick_i) = \text{SoftMax}(\frac{V_i}{Vi+Vj})$ where $V$ is the trust placed in an advisor and $_i$ and $_j$ are competing advisors.
...
All parameters ($\tau$, $\lambda$, $\beta$, and the $\text{SoftMax}$ temperature parameter) will be fit to minimise prediction error over the available trials.
...
d <- d %>%
  mutate(
    ...
    mdls = map(data, ~gradientDescent(., nStartingLocations = 15L))
  )
...
doGradDesc <- function(x) {
  ...
  x %>% mutate(mdls = purrr::map(
    data, 
    ~gradientDescent(., nStartingLocations = 15L))
  )
}

dList <- list()
nCores <- detectCores() - 4
nPerCore <- ceiling(nrow(d) / nCores)
for (i in 1:nCores) {
  rows <- (1 + (i - 1) * nPerCore):(if (i == nCores) nrow(d) else (i) * nPerCore)
  dList[[i]] <- d[rows, ] 
}
