...
I report the results of experiments designed to extend Pescetelli and Yeung's results to the domain of advisor influence.

\mccorrect{!TODO[move this section to a discussion somewhere?]} An alternative explanation for the data is that disagreeing advice is more surpising when it comes from an advisor who usually agrees, and that it is its increased salience increases its influence rather than increased trust in the advisor.
...
ggplot(tmp, aes(x = adviceTypeName, y = adviceInfluence, 
  ...
  scale_colour_discrete(h.start = 90) +
...
ggplot(tmp, aes(x = '', y = pChooseHigh)) +
  ...
  # annotate(geom = 'segment', x = .5, xend = 1.5, y = 1.05, yend = 1.05, colour = 'black') +
  annotate(geom = 'label', label = paste0('BF vs chance\n', num2str(exp(bf@bayesFactor$bf))),
           x = 1, y = 1.05) +
...
select_experiment(
  project = 'datequiz',
  function(x) filter(x, study == 'advisorChoice', replication)
)
...
ggplot(tmp, aes(x = feedbackCondition, y = pChooseAcc)) +
  ...
  geom_violinhalf(fill = 'grey85', size = 1) +
