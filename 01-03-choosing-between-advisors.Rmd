...
familiarization %>%
  mutate(
    Agree = if_else(advisorAgrees,
                    'Agree', 'Disagree'),
    `Initial decision correct` = if_else(initialCorrect, 'Correct', 'Incorrect'),
...
Familiarization %>%
  mutate(
    Agree = if_else(advisor0adviceSide == responseAnswerSide,
                    'Agree', 'Disagree'),
    `Initial decision correct` = if_else(responseAnswerSideCorrect, 
                                         'Correct', 'Incorrect'),
...
```{r ac-acc-dates-r-advice-agr, fig.caption="Advisor behaviour for Dates task with in/accurate advisors.  Faint lines show the average agreement rate of the advisors as experienced by an individual participant. Box plots and violins show the distribution of the participant means."}
