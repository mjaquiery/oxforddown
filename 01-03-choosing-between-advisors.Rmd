...
The model of advisor evaluation described earlier ([§Advisor evaluation without feedback](#advisor-evaluation-without-feedback)) requires empirical support.
...
The extent to which advice is taken ([§General method - Analysis - Dependent variables - Weight on Advice](#m-analysis-dv-woa)) is commonly used as a measure of a participant's trust in an advisor, on the argument that the participant seeks to maximise task performance and task performance is maximised by taking more advice from more trustworthy advisors.
...
Pescetelli and Yeung developed a more sophisticated model of advisor evaluation in which the increase in trust gained when an advisor agreed with the decision-maker was contingent upon the confidence of the decision-maker's initial estimate ([§Confidence weighted agreement](#confidence-weighted-agreement)).
...
tmp <- familiarization %>% 
  mutate(`Initial answer correct` = 
           if_else(initialAnswerCorrect, 'Correct', 'Incorrect')) %>%
  group_by(`Initial answer correct`, pid) %>%
  summarise(
    `Initial estimate` = mean(initialConfidence),
    `Final decision` = mean(finalConfidence),
    .groups = 'drop'
  ) %>%
  pivot_longer(cols = c(`Initial estimate`, `Final decision`), 
...
bf <- tmp %>% 
  ...
  mutate(d = map(d, as.data.frame),
         bf = map(d, ~ ttestBF(
           x = .$Confidence[.$`Initial answer correct` == 'Correct'],
           y = .$Confidence[.$`Initial answer correct` == 'Incorrect'], 
           data = ., paired = T)
         ),
         bf = map_chr(bf, ~ .@bayesFactor %>% .$bf %>% exp() %>% bf2str())) %>%
  select(-d)
...
tmp %>%
  mutate(`Initial answer correct` = fct_rev(`Initial answer correct`)) %>%
  ggplot(aes(x = `Initial answer correct`, y = Confidence, colour = Response)) +
  scale_y_continuous(limits = c(0, 100), expand = c(0, 0)) +
  scale_x_discrete() +
  coord_cartesian(clip = F) +
  geom_line(aes(group = pid), alpha = .25) +
  geom_split_violin(aes(x = nudge(`Initial answer correct`, dw), 
                        group = `Initial answer correct`, 
                        fill = Response), 
                    width = .9, colour = NA) +
  geom_boxplot(aes(x = nudge(`Initial answer correct`, dw), 
                   group = `Initial answer correct`),
               outlier.shape = NA, size = 1, width = dw/2, colour = 'black') +
  geom_segment(x = 1 - dw, xend = 2 + dw, y = 100, yend = 100, 
               colour = 'black') +
  geom_label(aes(label = paste0('BF = ', bf)), 
             x = 1.5, y = 100, colour = 'black', data = bf) +
  facet_wrap(~response)
...
tmp <- familiarization %>% 
  ...
  mutate(
    d = map(d, ~ p_conf(., 0:nQuantiles))
  ) %>%
  unnest(cols = d) %>% unnest(cols = d)

tmp <- tmp %>% 
  mutate(
    Response = fct_rev(if_else(Response == 'initial', 
...
tmp.avg %>%
  ...
  labs(x = 'p( Confidence | Incorrect )', y = 'p( Confidence | Correct )')
...
Both the Agreement and Disagreement facets are highly symmetrical, which is expected because there should be no differences between leftward and rightward responses or their revisions following advice.
As expected from participants who are paying attention to the task and attempting to maximize the calibration and accuracy of their final decision accuracy, when participants receive agreeing advice they tend to increase their confidence in their initial response. 
Likewise, where participants receive disagreeing advice, they tend to reduce their confidence in their answer. 
Somewhat surprisingly, but understandably, when participants change their decision (a relatively uncommon event) they quite often make their final decision with a confidence equivalent to their initial estimate, producing a distinctive cross pattern.

\mccorrect{Would heatmaps look better for this?}

```{r ac-acc-dots-r-confidence-change, fig.caption="Confidence change on the Dots task with in/accurate advisors.  Each point shows the initial and final confidence on a single trial, with increasingly negative values indicating a firmer decision in favour of the left box and increasingly positive values indicating a firmer decision in favour of the right box. Points close to the dashed y = x line indicate relatively little change, while points further away indicate relatively greater change. The shaded grey area shows the zone outside which influence is capped (by moving vertically towards the grey zone boundary) when using the capped influence measure. Agreement and disagreement trials are plotted seperately, with trials coloured according to whether the final decision was correct."}

# These polygon points define a parellelogram marking the limits for the capped influence
df.poly1 <- data.frame(    
  x = c(-Inf, 0, 0),
  y = c(-Inf, -Inf, Inf)
)
...
familiarization %>%
  mutate(
    Agree = if_else(advisorAgrees,
                    'Agree', 'Disagree'),
    `Final decision correct` = if_else(finalCorrect, 'Correct', 'Incorrect'),
    `Initial confidence` = if_else(initialAnswer == 0, 
                      initialConfidence * -1, 
                      initialConfidence),
    `Final confidence` = if_else(finalAnswer == 0, 
                      finalConfidence * -1, 
                      finalConfidence)
  ) %>%
  ...
  geom_point(alpha = 0.2, aes(color = `Final decision correct`), size = 1) +
  geom_abline(slope = 1, intercept = 0, linetype = 'dashed', size = 1, color = 'black') +
  scale_x_continuous(limits = c(-50,50), expand = c(0,1)) +
  scale_y_continuous(limits = c(-50,50), expand = c(0,1)) +
...
Figure \@reg(fig:ac-acc-dots-r-advice-agr) shows the agreement rates experienced by each participant. 
...
ggplot(tmp, aes(x = adviceTypeName, y = adviceInfluence, 
  ...
  geom_boxplot(outlier.shape = NA, size = 1, width = dw/2,
               ...
               colour = 'black') +
  geom_segment(x = 1, xend = 2, y = 100, yend = 100, colour = 'black') +
  geom_label(y = 100, x = 1.5, colour = 'black', fill = 'white', 
             aes(label = paste0('BF = ', bf))) +
  scale_y_continuous(limits = c(-50, 100), 
                     breaks = seq(-50, 100, length.out = 6),
                     expand = c(0, 0)) +
  coord_cartesian(clip = F) +
...
Participants selected the High accuracy advisor at a rate greater than would be expected if their choosing were random (`r .T`).
...
This study used the binary version of the Dates Task ([§Estimation (Dates Task) - Binary](#m-p-dates-b)).
...
tmp <- Familiarization %>% 
  ...
  group_by(`Initial answer correct`, pid) %>%
  summarise(
    `Initial estimate` = mean(responseConfidence),
    `Final decision` = mean(responseConfidenceFinal),
    .groups = 'drop'
  ) %>%
  pivot_longer(cols = c(`Initial estimate`, `Final decision`), 
               names_to = 'Response', values_to = 'Confidence') %>%
...
tmp %>%
  ...
  geom_segment(x = 1 - dw, xend = 2 + dw, y = 101, yend = 101, 
               colour = 'black') +
  geom_label(aes(label = paste0('BF = ', bf)), 
             x = 1.5, y = 101, colour = 'black', data = bf) +
  facet_wrap(~response)
...
\mccorrect{!TODO[We need some kind of solution here for calculating metacognitive sensitivity without being affected by metacognitive bias because we don't fix type 1 task performance. Meta-d' might work, if it can be calculated accurate over 10 or so observations. It may be that we can't reliably quantify participants' metacognitive sensitivity sensibly on a task with 10 or so trials and a 100-point confidence scale.]}
...
tmp <- Familiarization %>% 
  transmute(
    ...
    initialConfidence = responseConfidence,
    finalConfidence = responseConfidenceFinal
  ) %>%
  pivot_longer(cols = c(starts_with('initial'), starts_with('final')),
               names_to = c('Response', '.value'),
               names_pattern = '(initial|final)(.*)') %>%
...
\mccorrect{!TODO[Compare to the dots task version during the descriptive interpretation.]}
...
Familiarization %>%
  mutate(
    Agree = if_else(advisor0adviceSide == responseAnswerSide,
                    'Advisor agrees', 'Advisor disagrees'),
    `Final decision correct` = if_else(responseAnswerSideCorrectFinal, 'Correct', 'Incorrect'),
    `Initial confidence` = if_else(responseAnswerSide == 0, 
                      responseConfidence * -1, 
                      responseConfidence),
    `Final confidence` = if_else(responseAnswerSideFinal == 0, 
                      responseConfidenceFinal * -1, 
                      responseConfidenceFinal)
  ) %>%
  ggplot(aes(x = `Initial confidence`, y = `Final confidence`)) +
  geom_polygon(data = df.poly1, aes(x,y), fill = 'grey', alpha = 0.2) +
  geom_polygon(data = df.poly2, aes(x,y), fill = 'grey', alpha = 0.2) +
  geom_point(alpha = 0.2, aes(color = `Final decision correct`)) +
  geom_abline(slope = 1, intercept = 0, linetype = 'dashed', size = 1, color = 'black') +
  scale_x_continuous(limits = c(-100,100), expand = c(0,1)) +
  scale_y_continuous(limits = c(-100,100), expand = c(0,1)) +
  theme(panel.spacing = unit(2, 'lines'), 
        legend.position = 'bottom',
        plot.margin = unit(c(0, 0, 1, 0), 'lines')) +
  coord_fixed() +
...
```{r ac-acc-dates-r-advice-acc, fig.caption="Advisor behaviour for Dates task with in/accurate advisors.  Coloured lines show the average accuracy of the advisors as experienced by an individual participant. The colour of the line indicates whether the more accurate advisor was more accurate as per the experiment design. Box plots and violins show the distribution of the participant means, while the dashed lines indicate the accuracy level for the advisors specified in their design."}
...
```{r ac-acc-dates-r-advice-agr, fig.caption="Advisor behaviour for Dates task with in/accurate advisors.  Faint lines show the average agreement rate of the advisors as experienced by an individual participant. Box plots and violins show the distribution of the participant means, while the dashed lines indicate the agreement level for the advisors specified in their design."}
...
bf <- tmp %>% 
  ...
  mutate(
    bf = map(d, ~ select(., pid, advisor0idDescription, advisor0Influence) %>%
               pivot_wider(names_from = advisor0idDescription, 
                           values_from = advisor0Influence) %>%
               filter_if(is.numeric, ~ !is.na(.)) %>%
               as.data.frame() %$%
               ttestBF(x = highAccuracy, y = lowAccuracy, data = ., paired = T))
  ) 
...
ggplot(tmp, aes(x = advisor0idDescription, y = advisor0Influence, 
                colour = `Feedback condition`, fill = `Feedback condition`)) +
  geom_hline(yintercept = 0, colour = 'lightgrey', size = 1) +
  geom_line(aes(group = pid), alpha = .25) +
  geom_split_violin(aes(x = nudge(advisor0idDescription, dw),
                        group = advisor0idDescription), width = 1,
               colour = NA) +
  geom_boxplot(outlier.shape = NA, size = 1, width = dw/2,
               aes(x = nudge(advisor0idDescription, dw), 
                   group = advisor0idDescription),
               colour = 'black') +
  geom_segment(x = 1, xend = 2, y = 155, yend = 155, colour = 'black') +
  geom_label(y = 155, x = 1.5, colour = 'black', fill = 'white', 
             aes(label = paste0('BF = ', BF)), data = bf) +
  scale_y_continuous(limits = c(-50, 160), 
                     breaks = seq(-50, 150, length.out = 6)) +
...
This is a different result to that observed in the Dots task [§Dots task - Results - Hypothesis test](#ac-acc-dots-r-h), which also had no feedback.
Preferences were quite evenly distributed across the full range of directions and strengths, with a slight numerical advantage for the Low accuracy advisor.
...
acc <- agr
acc[2, ] <- 1 - acc[2, ]
acc <- sapply(1:ncol(acc), function(i) sum(acc[, i] * c(pCor, 1 - pCor)))
...
The High agreement advisor was correct `r acc[1] * 100`% of the time while the Low agreement advisor was correct `r acc[2] * 100`% of the time.
The advisor profiles were balanced for overall accuracy rates.

```{r ac-agr-dots-m-profiles} 

kable(tmp, caption = "\\label{tab:ac-agr-dots-m-profiles}Advisor advice profiles for Dots task Agreement experiment") %>%
  kable_styling() %>%
  column_spec(1, bold = T) %>%
  row_spec(0, bold = F) %>%
  collapse_rows(columns = 1, valign = "top") %>%
...
ggplot(tmp, aes(x = adviceTypeName, y = adviceInfluence, 
  ...
  geom_boxplot(outlier.shape = NA, size = 1, width = dw/2,
               aes(x = nudge(adviceTypeName, dw), 
                   group = adviceTypeName),
               colour = 'black') +
  geom_segment(x = 1, xend = 2, y = 105, yend = 105, colour = 'black') +
  geom_label(y = 105, x = 1.5, colour = 'black', fill = 'white', 
             aes(label = paste0('BF = ', bf))) +
  scale_y_continuous(limits = c(-50, 110), breaks = seq(-50, 100, length.out = 6)) +
  facet_grid(~`Feedback condition`) +
  labs(x = 'Advisor advice profile', y = 'Influence of advice') +
  broken_axis
...
The results were the same as those for the Dots task with High versus Low accuracy advisors.
...
tmp <- tribble(
  ...
  "High accuracy", agr[1, 1], agr[2, 1],
  "Low accuracy", agr[1, 2], agr[2, 2],
) %>% 
  prop2str()

kable(tmp, caption = "\\label{tab:ac-agr-dates-m-profiles}Advisor advice profiles for Dates task Agreement experiment") %>%
  kable_styling() %>%
  column_spec(1, bold = T) %>%
  row_spec(0, bold = F) %>%
  collapse_rows(columns = 1, valign = "top") %>%
...
bf <- tmp %>% 
  ...
  mutate(
    bf = map(d, ~ select(., pid, advisor0idDescription, advisor0Influence) %>%
               pivot_wider(names_from = advisor0idDescription, values_from = advisor0Influence) %>%
               filter_if(is.numeric, ~ !is.na(.)) %$%
               ttestBF(x = highAgreement, y = lowAgreement, data = ., paired = T))
  ) 
...
ggplot(tmp, aes(x = advisor0idDescription, y = advisor0Influence, 
  ...
  geom_label(y = 155, x = 1.5, colour = 'black', fill = 'white', 
             aes(label = paste0('BF = ', BF)), data = bf) +
  scale_y_continuous(limits = c(-50, 160), breaks = seq(-50, 150, length.out = 6)) +
  scale_fill_hue(direction = -1, aesthetics = c('fill', 'colour')) +
  facet_grid(~`Feedback condition`) +
  labs(x = 'Advisor advice profile', y = 'Influence of advice') +
  broken_axis
...
The modal preference was to select the High agreement advisor on every Choice trial, and although some participants still showed a preference for hearing advice from the Low agreement advisor, preferences for the High agreement advisor were generally stronger and more frequent.
...
kable(tmp, caption = "\\label{tab:ac-ava-dots-m-profiles}Advisor advice profiles for Dots task Accuracy/agreement experiment") %>%
  kable_styling() %>%
  column_spec(1, bold = T) %>%
  row_spec(0, bold = F) %>%
  collapse_rows(columns = 1, valign = "top") %>%
...
tmp <- trials %>% 
  ...
  select(c(matches('(adviceInfluence)'), group_vars(.))) %>%
  summarise_all(mean) %>%
  mutate(adviceTypeName = case_when(adviceType == 9 ~ 'High accuracy',
                                    adviceType == 10 ~ 'High agreement',
                                    T ~ NA_character_),
         `Feedback condition` = 'No feedback') %>%
  mutate(across(-matches('Influence'), factor))
...
bf <- tmp %>% 
  select(group_vars(.), adviceTypeName, adviceInfluence) %>%
  pivot_wider(names_from = adviceTypeName, values_from = adviceInfluence) %$%
  ttestBF(`High accuracy`, `High agreement`, paired = T) %>%
  .@bayesFactor %>%
  .$bf %>%
  exp() %>%
  bf2str()
...
```{r ac-ava-dates-r-influence-graph, fig.caption="Date task advisor influence for high accuracy/agreement advisors.  Participants' weight on the advice for advisors in the Familiarization phase of the experiment. The shaded area and boxplots indicate the distribution of the individual participants' mean influence of advice. Individual means for each participant are shown with lines in the centre of the graph. The dotted outline indicates the distribution of participant means in the original experiment of which this experiment is a replication. The theoretical range for influence values is [-200, 200]."}
...
for (E in c(.GlobalEnv, .prevStudy)) {
  ...
  E$tmp <- left_join(
    ...
    mutate_if(is.character, factor) %>%
    mutate(`Feedback condition` = feedbackCondition) %>%
    filter(!is.na(advisor0Influence))
}
...
bf <- tmp %>% 
  ...
  mutate(
    bf = map(d, ~ select(., pid, advisor0idDescription, advisor0Influence) %>%
               pivot_wider(names_from = advisor0idDescription, values_from = advisor0Influence) %>%
               filter_if(is.numeric, ~ !is.na(.)) %$%
               ttestBF(x = Agreeing, y = Accurate, data = ., paired = T))
  ) 
...
ggplot(tmp, aes(x = advisor0idDescription, y = advisor0Influence, 
  ...
  geom_boxplot(outlier.shape = NA, size = 1, width = dw/2,
               aes(x = nudge(advisor0idDescription, dw), 
                   group = advisor0idDescription),
               colour = 'black') +
  geom_segment(x = 1, xend = 2, y = 45, yend = 45, colour = 'black') +
  geom_label(y = 45, x = 1.5, colour = 'black', fill = 'white', 
             aes(label = paste0('BF = ', BF)), data = bf) +
  scale_y_continuous(limits = c(-10, 50), breaks = seq(-10, 40, length.out = 6)) +
  scale_fill_hue(direction = -1, aesthetics = c('fill', 'colour')) +
  facet_grid(~`Feedback condition`) +
  labs(x = 'Advisor advice profile', y = 'Influence of advice') +
  broken_axis
...
\mccorrect{!TODO[This is really the critical test - can we show the same crossover pattern with feedback that we saw in Taking Advice]}

## Confidence-contingent advice {#ac-cca}

Pescetelli and Yeung's model of advisor evaluation ([§Confidence weighted agreement](#confidence-weighted-agreement)) of advisor evaluation weights the updating of trust in an agreeing advisor using the confidence of the judge's initial estimate.
...
kable(tmp, caption = "\\label{tab:ac-cca-dots-profiles}Confidence-contingent advisor advice profiles") %>%
  kable_styling() %>%
  column_spec(1, bold = T) %>%
  row_spec(0, bold = F) %>%
  collapse_rows(columns = 1, valign = "top") %>%
...
tmp <- trials %>% 
  filter(hasChoice) %>%
  group_by(pid) %>%
  summarise(pChooseBiasSharing = sum(adviceType == 3) / sum(adviceType %in% c(3,4))) %>%
  mutate(`Feedback condition` = 'No feedback')

bf <- ttestBF(pull(tmp, pChooseBiasSharing), mu = .5)

ggplot(tmp, aes(x = '', y = pChooseBiasSharing)) +
  geom_hline(yintercept = .5, linetype = 'dashed') +
  geom_violindot(size_dots = .4) +
  geom_violinhalf(aes(fill = `Feedback condition`, colour = `Feedback condition`)) +
  annotate(geom = 'label', label = paste0('BF vs chance\n', bf2str(exp(bf@bayesFactor$bf))),
...
To redress this balance, a capped measure of influence ([§Method - Analysis - Capped influence](#ex1-m-analysis-influence)) was used.
...
Analysis using the 2x2x2 (advice type/agreement/choice) ANOVA described above ([§Advisor influence](#ex1-influence)) no longer showed a significant effect of agreement.
...
Participants in the Dots task had a systematic preference for the High accuracy advisor ([§Accuracy - Dots task](#ac-acc-dots)), while those in the No feedback condition of the Dates task did not ([§Accuracy - Dates task](#ac-acc-dates)).
