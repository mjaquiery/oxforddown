...
exclusions %>% 
  ...
  kable(caption = "Table \\label{tab:ac-acc-dots-exclusions}: Participant exclusions for Dots task Accuracy experiment") %>%
...
exclusions %>% 
  ...
  pivot_longer(everything(), names_to = "Reason", values_to = "Participants excluded") %>%
  kable(caption = "Table \\label{tab:ac-acc-dates-exclusions}: Participant exclusions for Dates task Accuracy experiment") %>%
  row_spec((n - 1):n, bold = T)
...
bf.comp <- ttestBF(formula = pChooseHigh ~ `Feedback condition`, data = tmp) %>%
...
kable(tmp, caption = "Table \\label{tab:ac-agr-dots-m-profiles}: Advisor advice profiles for Dots task Agreement experiment") %>%
  kable_styling() %>%
  column_spec(1, bold = T) %>%
  row_spec(0, bold = F) %>%
  collapse_rows(columns = 1, valign = "top") %>%
...
exclusions %>% 
  ...
  mutate(`Total remaining` = length(unique(trials$pid))) %>% 
  pivot_longer(everything(), names_to = "Reason", values_to = "Participants excluded") %>%
  kable(caption = "Table \\label{tab:ac-agr-dots-exclusions}: Participant exclusions for Dots task Agreement experiment") %>%
  row_spec((n - 1):n, bold = T)
...
exclusions %>% 
  ...
  mutate(`Total remaining` = length(unique(AdvisedTrial$pid))) %>% 
  pivot_longer(everything(), names_to = "Reason", values_to = "Participants excluded") %>%
  kable(caption = "Table \\label{tab:ac-agr-dates-exclusions}: Participant exclusions for Dates task Agreement experiment") %>%
  row_spec((n - 1):n, bold = T)
...
The High versus Low agreement advisor experiment showed that participants preferred to receive agreeing advice \mccorrect{when they did not receive feedback.}
...
The High accuracy advisor was correct 80% of the time (estimated agreement = `r (.8 * .71 + .2 * .29) * 100`%) while the High agreement advisor agreed with the participant 80% of the time (estimated accuracy = `r (.8 * .71 + .2 * .29) * 100`%). 
...
exclusions %>% 
  ...
  mutate(`Total remaining` = length(unique(trials$pid))) %>% 
  pivot_longer(everything(), names_to = "Reason", values_to = "Participants excluded") %>%
  kable(caption = "Table \\label{tab:ac-ava-dots-exclusions}: Participant exclusions for Dots task Accuracy vs Agreement experiment") %>%
  row_spec((n - 1):n, bold = T)
...
exclusions %>% 
  ...
  mutate(`Total remaining` = length(unique(AdvisedTrial$pid))) %>% 
  pivot_longer(everything(), names_to = "Reason", values_to = "Participants excluded") %>%
  kable(caption = "Table \\label{tab:ac-ava-dates-exclusions}: Participant exclusions for Dates task Accuracy vs Agreement experiment") %>%
  row_spec((n - 1):n, bold = T)
...
```{r ac-ava-dates-r-influence-graph, fig.caption="Date task advisor influence for high accuracy/agreement advisors.  Participants' weight on the advice for advisors in the Familiarization phase of the experiment. The shaded area and boxplots indicate the distribution of the individual participants' mean influence of advice. Individual means for each participant are shown with lines in the centre of the graph. The dotted outline indicates the distribution of participant means in the original experiment of which this experiment is a replication. The theoretical range for influence values is [-200, 99]."}
...
The Anti-bias advisor agrees more frequently with participants when their initial estimate is correct and made with relatively low confidence \@ref(tab:ac-cca-dots-profiles).
...
kable(tmp, caption = "Table \\label{tab:ac-cca-dots-profiles}: Confidence-contingent advisor advice profiles") %>%
  kable_styling() %>%
  column_spec(1, bold = T) %>%
  row_spec(0, bold = F) %>%
  collapse_rows(columns = 1, valign = "top") %>%
...
exclusions %>% 
  ...
  mutate(`Total remaining` = length(unique(trials$pid))) %>% 
  pivot_longer(everything(), names_to = "Reason", values_to = "Participants excluded") %>%
  kable(caption = "Table \\label{tab:ac-cca-dots-exclusions}: Participant exclusions for Dots task Confidence-contingent agreement experiment") %>%
  row_spec((n - 1):n, bold = T)
