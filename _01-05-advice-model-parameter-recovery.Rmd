...
#' @param expand_confidence whether to scale the confidence values such that the highest recorded confidence value is 100% and the lowest is completely unsure
#' @param noise standard deviation of noise added to initial and final confidence.
#'   On the same scale as scale_width. 
#'
...
simulate_participant <- function(
  ...
  #' Cap x +/- rnorm(sd = n) values to [-w, w] and nudge 0 values to 1
  .smear <- function(x, n, w) {
    x <- pmin(w, pmax(-w, rnorm(length(x), x, n)))
    x <- round(x)
    x[x == 0] <- 1
    x
  }
  #' Conversion functions between scale values, probabilities, and z-expanded probabilities. These don't do much here but we'll perhaps use them in model fitting.
  .scale_to_p <- function(x, lim) abs(x / lim / 2) + .5
  .p_to_scale <- function(x, lim) x * lim * 2 - lim
  .p_to_pz <- function(x) {
    x <- scale(x)
    attr(x, 'min') <- min(x)
    attr(x, 'max') <- max(x)
  ...
  .pz_to_p <- function(x) {
    x <- x * (attr(x, 'max') - attr(x, 'min')) + attr(x, 'min')
  ...
  convert_in <- 
    if (expand_confidence) function(x) .p_to_pz(.scale_to_p(x, scale_width)) 
    else function(x) .scale_to_p(x, scale_width)
  convert_out <- 
    if (expand_confidence) function(x) .p_to_scale(.pz_to_p(x), scale_width) 
    else function(x) .p_to_scale(x, scale_width)
  
  # tests for conversion functions - need a better place for these
  .test_vector <- rnorm(1000, 25, 3)
  testthat::expect_equal(
    round(.test_vector, 9), 
    round(convert_out(convert_in(.test_vector)), 9)
  )
  ...
  out$initialConfidence <- .smear(out$initialConfidence, noise, scale_width)
  ...
  # $$c_2 = (c_1 * t) / (c_1 * t - (1-c_1)(1-t))$$
  c1 <- convert_in(out$initialConfidence)
  ...
  # Rescale
  out$finalConfidence <- convert_out(c2.hat)
  out$finalConfidence <- .smear(out$finalConfidence, noise, scale_width)
...
visualiseRun <- function(x, scale_width = 55) {
  d <- x  %>%
    # replace trial id with trial so scrambles work
...
```{r demo case}
demo <- list(tu = .25, ws = 3, n = 15, noise = 0)
...
for (i in 1:nrow(simulants)) {
  tryCatch(
    ...
    error = \(e) warning(paste0('Error as warning:\n', e))
  )
}
...
for (p in unique(tmp$parameter)) {
  lim <- tmp %>% 
    ...
    summarise(low = max(min(value), -100), high = max(value))
  print(
    ggplot(
...
sims <- bind_cols(
  tibble(uid = 1:35),
  crossing(
    weightedSelection = seq(-6, 18, 4),
    trustUpdateRate = seq(0.1, 1, .2)
  )
) %>%
  nest(params = -uid) %>%
  mutate(data = map(params, simulate_participant, nTrials = 30)) %>%
...
for (i in 1:nrow(sims)) {
  print(visualiseGrid(
    gridSearch(sims[i, ], grid = tidyr::crossing(a = seq(-10, 30, .5),
                                          b = seq(-.1, 1.5, .1))), 
    target_value = sims[i, c('trustUpdateRate', 'weightedSelection')]
  ) + labs(title = 'sim'))
  print(visualiseGrid(
    gridSearch(shuffled_sims[i, ], grid = tidyr::crossing(a = seq(-10, 30, .5),
                                          b = seq(-.1, 1.5, .1))),
    target_value = sims[i, c('trustUpdateRate', 'weightedSelection')]
  ) + labs(title = 'sim_suffle'))
}
...
for (s in 1:nrow(sims)) {
  for (type in c('sims', 'shuffled_sims')) {
    sim <- get(type)[s,  ] %>%
      select(uid, data) %>%
      unnest(cols = data)
...
sims <- tibble(
  ...
  weightedSelection = 5,
  trustUpdateRate = .3,
  noise = c(0, 1, 2, 3, 5, 10, 15, 25, 40, 55)
) %>%
  nest(params = c(-uid, -noise)) %>%
  mutate(data = map(params, simulate_participant, nTrials = 30, noise = noise)) %>%
  unnest(cols = params)
  ...
  tryCatch(
    {
      print(visualiseGrid(
        gridSearch(sims[i, ], grid = tidyr::crossing(a = seq(-10, 30, .5),
                                                     b = seq(-.1, 1.5, .1))), 
        target_value = sims[i, c('trustUpdateRate', 'weightedSelection')]
      ) + labs(title = glue('sim(noise = {sims$noise[i]})')))
      print(visualiseGrid(
        gridSearch(shuffled_sims[i, ], grid = tidyr::crossing(a = seq(-10, 30, .5),
                                                              b = seq(-.1, 1.5, .1))),
        target_value = sims[i, c('trustUpdateRate', 'weightedSelection')]
      ) + labs(title = glue('sim_suffle(noise = {shuffled_sims$noise[i]})')))
    },
    error = \(e) warning(paste0('Error as warning:\n', e))
  )
}
    ...
    tryCatch(
      {
        ...
        results <- bind_rows(
          results,
          best %>% mutate(coefs = 'best', source = type, noise = get(type)$noise[s]), 
          tibble(
            uid = sim$uid[1],
            ws_end = sim$weightedSelection[1],
            tu_end = sim$trustUpdateRate[1],
...
tmp <- results %>% 
  select(uid, ws_end, tu_end, coefs, source, noise) %>%
  pivot_longer(matches('_end'), names_to = 'parameter', values_to = 'coefficient') %>%
  pivot_wider(names_from = coefs, values_from = coefficient)
...
for (i in 1:nrow(sims)) {
  tryCatch(
    {
      x <- gridSearch(sims[i,], grid = tidyr::crossing(a = seq(-15, 30, .5), b = fixed_TUR))
      x.min <- filter(x, weightedSelection_error == min(weightedSelection_error))
      best <- bind_rows(
        best, 
        tibble(
          uid = x.min$uid,
          best = x.min$weightedSelection,
          real = x.min$data[[1]]$weightedSelection[1]
        )
      )
      
      print(
        ggplot(x, aes(x = weightedSelection, y = weightedSelection_error)) +
          geom_point(
            aes(
              x = x$data[[1]]$weightedSelection[1], 
              y = mean(weightedSelection_error)
            ),
            colour = 'red', 
            shape = 8, 
            size = 6
          ...
          geom_point() + 
          geom_vline(aes(xintercept = weightedSelection), data = x.min) +
          geom_label(
            aes(
              x = weightedSelection, 
              y = mean(x$weightedSelection_error), 
              label = weightedSelection
            ),
            data = x.min
          )
      )
