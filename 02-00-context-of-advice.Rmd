...
Most experiments which explore egocentric discounting use the Judge-Advisor System.
The Judge-Advisor System has roles for a judge, usually the participant, and one or more advisors, often another participant.
The judge offers an initial estimate for some decision, e.g. the value of coins in a change jar, then receives advice from the advisor, and then makes a final decision. 
...
When combining individual estimates from multiple distributions, optimal results are obtained by weighting the estimates according to the relative precision of their parent distributions [@sollStrategiesRevisingJudgment2009].
...
It is not a generic inability to combine estimates: people can combine estimates which do not include their own opinion well [@yanivExploitingWisdomOthers2012], even adjusting for differences in ability between advisors [@sollJudgmentalAggregationStrategies2011].
Similarly, @troucheVigilantConservatismEvaluating2018 showed that discounting occurred _towards advice_ when that advice was labelled as the judge's initial estimate and vice-versa, suggesting that a person's own opinion has a privileged status.

In this chapter, I review the literature on egocentric discounting, with particular attention to manipulations that have been used and explanations which have been offered.
...
A sizeable body of research has been conducted into egocentric discounting, including a wide variety of manipulations. 
...
While egocentric discounting and and advice weighting are inversely related, establishing the degree to which advice has been discounted requires a reference value from which its weighting has deviated.
Many of the experiments do not calculate this normative value, nor even give sufficient details to establish a reference value, either an objective normative value or an expected value from the perspective of the participant.
It should be born in mind, therefore, that depending on the circumstances, very high or low levels of advice weighting might not correspond to very low or high levels of discounting: when @schultzeInabilityIgnoreUseless2017 provide participants with estimates from a random number generator any level of advice weighting corresponds to negative egocentric discounting; and in many other experiments where expert advice is provided even weighting advice evenly corresponds to discounting.
...
In both cases, partial or full mediation through confidence occurred such that higher confidence was associated with greater egocentric discounting.
...
Discounting was still evident in this process, as it was in @vanswolForecastingAnotherEnjoyment2011, which used a traditional Judge-Advisor System paradigm where advice was delivered face-to-face.
...
The most frequently-manipulated property of advice, and the most interesting in the context of the first part of this thesis, is the similarity of the advice to the initial estimate. 
This is sometimes expressed as agreement or reasonableness of advice.
The evidence on the effects of advice distance on advice influence is equivocal: some studies appear to demonstrate that advice is discounted more the further it is from the initial estimate, while other studies show a greater influence of more distant advice.
...
@moussaidSocialInfluenceCollective2013 identified a three-zone structure to the influence of advice according to distance. 
Similar advice fell into the 'confirmation zone', where opinion was unchanged but confidence increased; moderately distant advice fell into an 'influence zone' where averaging strategies come into play; and distant advice was generally ignored.
Likewise, @schultzeEffectsDistanceInitial2015 showed in an elegant series of Judge-Advisor System experiments that relationships between egocentric discounting and advice distance were non-linear. 
Advice was discounted heavily where it was close to the initial estimate, with discounting then dropping precipitously to a peak for middle-distance advice, after which it either remained stable or decreased slightly as the distance increased further.
...
@hutterSeekingAdviceSampling2016 found consistently higher influence for advice which was further from the initial estimate both for single pieces of advice and for integrating multiple pieces of advice.
Hütter & Ache used three discrete advice distances which may explain why they found monotonic increases in influence while Moussaïd et al. and Schultze et al. found U-shaped relationships: Hütter & Ache may not have sampled a broad enough swathe of the distance-space.

The argument that close advice updates confidence rather than judgement may offer an explanation of the more straightforward monotonic reduction in advice-taking with distance.
This would be consistent with the results of @yanivUsingAdviceMultiple2007 if we assume that this confidence reduces advice influence ([§Task properties](#coa-ed-task)) and this reduction of influence disproportionately affects distant advice.
@rollwageConfidenceDrivesNeural2020 argue that high-confidence decisions reduce the processing of disconfirmatory evidence, and such a process may explain the above phenomenon: near advice inflates confidence, which then disproportionately hampers the processing of far advice, leading to an apparent monotonic decrease in advice influence with distance.
These mechanisms do not suffice to explain the results in @yanivReceivingOtherPeople2004, however, because there is only a single piece of advice which cannot be both near and far simultaneously.
The @yanivReceivingOtherPeople2004 results stand as something of a mystery, especially because @schultzeEffectsDistanceInitial2015 obtained results consistent with their other results in an experiment designed to reproduce Yaniv's methodology as closely as possible.
...
@ginoEffectsTaskDifficulty2007 compared advice-taking behaviour across two experiments in which the only difference in design was that participants in one received advice automatically and participants in the other had the option of clicking a button to receive advice.
@ginoEffectsTaskDifficulty2007 saw participants opting to receive advice on almost all trials, including in the easy condition, but saw no differences in the extent to which advice was used between the compulsory and optional advice experiments.
...
In the real world price is often a noisy indicator of quality, and solicitation is likely to act as a proxy for confidence, which is again related to task difficulty: participants in the experiments may have taken more advice because they were less sure on the trials in which they sought advice.
...
@sniezekImprovingJudgementPrepaid2004 saw greater dependence on advice from specially-trained 'expert' peers, but only where the proportion of reward money for accurate judgement paid to the advisor had been decided _before_ advice was provided.
...
@onkalEvaluatingExpertAdvice2017 provided a series of experiments using a stock price estimation task in which advisor expertise was manipulated using both labels and experience. 
...
Likewise, @mahmoodiEqualityBiasImpairs2015 showed that dyads making perceptual decisions would consistently over-weight the estimate of the least accurate dyad member despite evidence that this impaired performance and thus decreased the dyad's reward money.
...
For all these authors, a judge's confidence in the initial estimate is the principle driving force behind the weighting of advice.
...
One of the most influential explanations has been Yaniv's argument that judges have greater access to the reasons justifying their own decisions than to those justifying the decisions of others due to the opaqueness of other minds [@yanivAdviceTakingDecision2000].
This differential access to reasons suggests, from the perspective of the judge, that there is greater evidence favouring the judge's own opinion than favouring the estimate of their advisor, and that, analogously to the confidence case above, the more well-supported opinion should be given a greater weight during integration.
...
@bonnerGroupJudgmentAdviceTaking2014 suggested that anchoring was responsible for judges' over-use of outlandishly extravagant suggestions for CEO reumneration.
...
Another general cognitive bias recruited as an explanation for egocentric discounting is the sunk costs fallacy in which one perseveres with a poor strategy in order to justify the effort which has already gone into pursuing it.
Interestingly, sunk costs have been recruited to explain both egocentric discounting _and_ following advice.

@ginoWeListenAdvice2008 had participants receive advice for free or at a fee depending upon the outcome of a coin flip and found that the same advice was more influential where payment had been taken, and that the more expensive the more influential it was whether paid for or free.
...
This is an intriguing prediction, but because effortfulness tends to covary with difficulty (which in turn decreases egocentric discounting) it does not appear to have been studied.
...
While distribution of responsibility is more a reason for _reduced_ rather than _increased_ egocentric discounting, when combined with an account which predicts ascribing very low or no weight to advice by default (e.g. naïve realism), it can explain why advice weighting is higher than the zero that would be expected.
...
@troucheVigilantConservatismEvaluating2018 offer designate the above explanations 'proximal explanations', because they offer a mechanistic account of how discounting occurs.
...
They suggest that discounting occurs because advisors' interests do not always align with judges', and thus some level of discounting offers protection from relying too heavily on advice which may be deliberately harmful.
...
I argue that the normative model of advice-taking, and the paired down experimental design with which is it entwined, seek to take the situation out of the evolutionary history of advice, but cannot take the evolutionary history of advice out of the situation.

The hyperpriors on advice-taking are a consequence of the many opportunities for deception and misunderstanding which apply to advice but not to one's own opinion.
...
Consider, for example, a situation where one coworker, Sally, asks another, Hanan, whether it is a good idea to apply for a prompotion.
...
It is not necessary, however, for there to be misaligned incentives.
...
Information can be degraded at any of these steps, resulting in advice which is less informative than the judge's own opinion.
...
In the chapters which follow, I present evidence from computational agent-based evolutionary simulations and online human behavioural experiments to illustrate the plausibility of the claims made above. 
