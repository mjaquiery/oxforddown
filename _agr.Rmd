...
Pescetelli and Yeung \mccorrect{NICK: Niccolo Exp 2 in the JEP:G paper. He found (in terms of influence) a big effect of agreement and a much smaller effect of accuracy when there was no feedback. When feedback was provided, accuracy had maybe a bit bigger effect than agreement, but both effects were clear.} demonstrated that advisors who agree  more frequently are more influential (regardless of the presence of feedback) in a lab-based perceptual decision-making task.

\mccorrect{NICK: Here we explored the impact of agreement on choice of advisor. Experiment X looks at this effect in the Dots task, while Experiment Y does the same for the Dates task.}

\mccorrect{NICK: somewhere we need to talk about what we mean by agreement, how Niccolo defined it, how we define it (varies between binary/continuous tasks), etc. Remind the reader what we mean by agreement in Method}
...
\mccorrect{NICK: In this experiment, the advice provided was no longer independent of initial decisions. Instead, advice was provided contingent on those decisions, such that the two advisors varied in their likelihood etc. I think it's worth making the point explicit, to begin to suggest how we can dissociate agreement and accuracy.}
...
# Marginal means
out$acc_mmTime <- Familiarization %>% 
  rename(
...
out$aov_conf <- out$aov_conf$ANOVA %>%
  nest(d = -Effect) %>%
  mutate(s = map_chr(d, ~ glue(
    "F({.$DFn},{.$DFd})={num2str(.$F, 2)}, p={prop2str(.$p, minPrefix = '<')}"
  )))
...
# Marginal means
out$conf_mmTime <- Familiarization %>% 
  rename(
...
out$conf_mmInitialCor <- Familiarization %>%
...
out$conf_mmInt <- Familiarization %>% 
  mutate(
    initialCor = if_else(responseAnswerSideCorrect, "Correct", "Incorrect")
  ) %>%
  group_by(pid, initialCor) %>%
...
out$inf_aov <- out$inf_aov$ANOVA %>%
  nest(d = -Effect) %>%
  mutate(s = map_chr(d, ~ glue(
    "F({.$DFn},{.$DFd})={num2str(.$F, 2)}, p={prop2str(.$p, minPrefix = '<')}"
  )))

out$inf_mmAdvisor <- Familiarization %>% 
  group_by(pid, Advisor) %>%
...
out$inf_mmCondition <- Familiarization %>% 
  group_by(pid, Feedback) %>%
  summarise(inf = mean(advisor0Influence), .groups = "drop") %>%
  group_by(Feedback) %>%
  summarise(
    s = md.mean(inf, label = glue("M~Influence|{Feedback}~"), decimals = 2),
    .groups = "drop"
  ) %>%
  unique()
...
Participants' accuracy (Figure \@ref(fig:ac-agr-dates-r-performance)A), which was uncontrolled in this task, was greater on final decisions than on initial estimates (`r out$aov_acc$s[2]`; `r out$acc_mmTime %>% pull(s) %>% paste(collapse = ", ")`).
There was no significant difference between advisors (`r out$aov_acc$s[1]`), but the increase in final decision accuracy was greater for the Low agreement advisor than the High agreement advisor (`r out$aov_acc$s[3]`; `r out$acc_mmInt %>% pull(s) %>% paste(collapse = ", ")`).

As expected, and as shown in Figure \@ref(fig:ac-agr-dates-r-performance)B participants were systematically more confident when their initial estimate was correct as compared to incorrect (`r out$aov_conf$s[1]`; `r out$conf_mmInitialCor %>% pull(s) %>% paste(collapse = ", ")`).
Participants were also more confident on final decisions than on initial estimates (`r out$aov_conf$s[2]`; `r out$conf_mmTime %>% pull(s) %>% paste(collapse = ", ")`). 
This increase in confidence was greater when the initial estimate was correct as compared to incorrect (`r out$aov_conf$s[3]`; `r out$conf_mmInt %>% pull(s) %>% paste(collapse = ", ")`).

The High agreement advisor was more influential (Figure \@ref(fig:ac-agr-dates-r-performance)C) than the Low agreement advisor overall (`r out$aov_inf$s[2]`; `r out$inf_mmAdvisor %>% pull(s) %>% paste(collapse = ", ")`), and participants in the No feedback condition were more influenced than participants in the Feedback condition (`r out$aov_inf$s[1]`; `r out$inf_mmCondition %>% pull(s) %>% paste(collapse = ", ")`).
There was no evidence of an interaction between these differences, however (`r out$aov_inf$s[3]`).
